...
export class ControllerRegistry {
  ...
  readonly drawerController?: DrawerController;       // Only if drawer plugin present
  readonly timeSliderController?: TimeSliderController; // Only if time-slider plugin present
...
│   │   ├── ui-controller.ts                    # UI domain → store
│   │   ├── layer-controller.ts                 # Layer domain → store
│   │   ├── layer-set-controller.ts             # Map events → feature info queries
│   │   ├── data-table-controller.ts            # Data table operations
│   │   ├── drawer-controller.ts                # Drawer plugin (conditional)
│   │   └── time-slider-controller.ts           # Time slider plugin (conditional)
│   └── stores/

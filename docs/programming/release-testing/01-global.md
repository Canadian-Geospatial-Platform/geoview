...
| --- | ------------------- | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- | ---- |
| [ ] | Panel full screen   | Footer bar panel expands to fill viewer | 1. Open each footer bar panel (Legend, Layers, Data Table, Details, Time Slider, Chart) 2. Click the full screen button 3. Press ESC or click again to exit | Panel expands to fill the viewer; ESC exits full screen | M    |
| [ ] | Viewer full screen  | Entire map fills browser window         | 1. Click the viewer full screen button 2. Exit with ESC                                                                                                     | Map + footer bar fills the browser window               | M    |
...
|     | Test                 | Description                            | Steps                                                                                       | Expected Result                                | Auto |
| --- | -------------------- | -------------------------------------- | ------------------------------------------------------------------------------------------- | ---------------------------------------------- | ---- |
| [ ] | Details → Chart      | Cross-panel shortcut to Geochart       | 1. Open Details panel 2. Select a layer with geochart data 3. Click the chart shortcut icon | Geochart panel opens for that layer            | M    |
...
| [ ] | Guide persistence     | Guide stays open across panel switches | 1. Open any panel 2. Open Guide 3. Switch to another panel | Guide stays open (or reopens consistently) | M    |

## Cross-Panel Layer Visibility

|     | Test            | Description                            | Steps                                                                                               | Expected Result                                                                   | Auto |
| --- | --------------- | -------------------------------------- | --------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | ---- |
| [ ] | Visibility sync | Hiding layer propagates to all panels  | 1. Turn off visibility of a layer in the Layers panel 2. Check Legend, Data Table, Details, and Map | Legend icon greyed out; Data Table empty; Details not queryable; Map layer hidden | C    |
| [ ] | Visibility on   | Showing layer propagates to all panels | 1. Turn the layer back on 2. Check all panels                                                       | Layer reappears in Legend, Data Table, Details, and Map                           | C    |
...
| [ ] | Keyboard navigation   | Arrow keys navigate results   | 1. Use arrow keys to navigate result list 2. Press Enter to select | Selection zooms map                                     | M    |
| [ ] | Language switch       | Results update on lang change | 1. Switch language while results are displayed                     | Results update to the new language                      | M    |
...
| [ ] | URL contains state      | URL encodes map state params     | 1. Click share 2. Inspect the URL          | URL includes `p=`, `z=`, `c=`, `b=` parameters                   | C    |
| [ ] | URL contains layers     | URL includes geocore UUIDs       | 1. Click share 2. Inspect `keys=` param    | `keys=` includes valid geocore UUIDs from loaded layers          | C    |
...
| [ ] | Message types     | Correct icons per type        | 1. Generate different message types 2. Open panel | Correct icons: success=green, error=red, warning=orange, info=blue | M    |
...
| [ ] | Date formats update | Dates follow locale           | 1. Switch language 2. Check time slider and details panel | Date displays use the new locale format                         | M    |

## Two-Map Shortcuts

|     | Test                  | Description                  | Steps                                                                                    | Expected Result                                              | Auto |
| --- | --------------------- | ---------------------------- | ---------------------------------------------------------------------------------------- | ------------------------------------------------------------ | ---- |
| [ ] | Correct map targeting | Shortcuts target correct map | 1. Load page with 2 maps 2. Use shortcuts (chart, time slider, data table) from each map | Shortcut navigates to the correct map's panel, not the other | M    |

...
## Hidden Layers in Component Panels

Config: [`configs/navigator/demos/08-hidden-layer-lists.json`](../../../packages/geoview-core/public/configs/navigator/demos/08-hidden-layer-lists.json), shared by **Map 11** in [the test page source](../../../packages/geoview-core/public/templates/release-testing/rt-08-layers.html).

This config retains the four-layer custom slider from `12-package-time-slider-custom.json` and the Airborne chart definition from `13-package-geochart.json`. The Airborne chart uses station fields `OBJECTID` and `Location_Emplacement`, the related table at service layer `3`, and the existing `CollectionStart_DebutPrelevement`, `Activity_Activite_mBqm3`, and `Radionuclide_Radionucleide` fields. Only its configured station path is nested under a new explicit group.

**Custom slider identity:** The single entry is backed by `fire_progression/0` (list row **fire_progression_20230915**); its right-panel title is **Progression and Perimeters of Fires and Floods**. The other backing paths are `fire_perimeters/1`, `floods_canada/1`, and `historical-flood/0`. Inspect the row by its main path rather than expecting the custom title to replace the layer name in the list.

**Setup/reset before each row:** On Map 11, wait for all five leaf layers and both plugins to load. In Layers, turn on all leaves and **Airborne chart group**; keep the custom slider's time filtering **off** and playback stopped. Use the Layers panel's zoom-to-layer/visible-scale controls if needed so the tested layers are in range. Do not change zoom during a hide/show comparison: out-of-range filtering is not this test. For Details and Chart, click a rendered **Airborne stations** point and wait for a nonzero station result; for Data Table, select **Airborne stations** and wait for its table to finish loading. For Chart, also wait for an actual station chart to render before hiding. Do not click the map again between hiding and restoring (preserve query results). A failed service, missing backing layer, or empty baseline chart is **blocked**, not a pass. Restore visibility and leave filtering off after each row.

**Pass/fail evidence:** Inspect the active footer panel inside `#map11` in the browser's Elements/Accessibility tools. The **Available layers** and **Hidden layers** headings label separate native `ul` lists through `aria-labelledby`; real layer entries are `li` children. Check list membership in the rendered DOM, not just Zustand flags or the Layers/Legend tree. An empty Available list displays an instructions row, which is not a layer. Hidden heading/list disappear when there are no hidden entries. Use the expanded footer Details panel, not the app-bar compact icon view.

| Test                                                         | Description                                                        | Steps                                                                                                                                                                                                                                                                                                                                                                                                                         | Expected Result                                                                                                                                                                                                                                                                                                                                                                                                        | Auto |
| ------------------------------------------------------------ | ------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| Details Available/Hidden lists                               | Query-result row moves between real lists                          | 1. On Map 11, complete setup and select Airborne stations in Details<br>2. In Layers, hide only Airborne stations, then reopen Details<br>3. Inspect both labelled lists and click the hidden row text<br>4. Click its inline eye                                                                                                                                                                                             | Airborne stations exists exactly once under Hidden, not Available; its previous selection/content is cleared. The grey/italic row text does nothing. The eye returns it to Available, removes its hidden eye, and selects its retained feature content; no duplicate row remains                                                                                                                                       | M    |
| Data Table Available/Hidden lists                            | Loaded table row remains discoverable when hidden                  | 1. On Map 11, load and select the Airborne stations data table<br>2. Hide only Airborne stations in Layers, then reopen Data Table<br>3. Inspect the lists and click the hidden row text<br>4. Click its inline eye                                                                                                                                                                                                           | The row moves from Available to Hidden rather than disappearing; the selected table clears to the guide. Row text cannot reopen a hidden table. The eye restores visibility, moves the row back exactly once, and opens the table with retained data                                                                                                                                                                   | M    |
| Chart Available/Hidden lists                                 | Grouped Airborne chart entry survives a leaf toggle                | 1. On Map 11, query a station and render its Chart<br>2. Leave Airborne chart group on but hide Airborne stations in Layers<br>3. Reopen Chart and inspect both labelled lists<br>4. Click the hidden entry's eye                                                                                                                                                                                                             | The grouped child chart entry appears once under Hidden; the previous chart selection clears to the guide. Available shows its empty-state instructions if no other chart is available. The eye restores the child, moves it to Available, and displays the station chart again; no orphan or duplicate entry remains                                                                                                  | M    |
| Custom slider stays Available with any visible backing layer | Main-layer visibility alone must not decide membership             | 1. On Map 11, select the custom slider entry backed by fire_progression/0 in Time Slider<br>2. In Layers, hide fire_progression/0 (the main path), fire_perimeters/1, and floods_canada/1, leaving only historical-flood/0 on; reopen Time Slider<br>3. Restore all four; then hide the three additional paths, leaving only fire_progression/0 on; reopen Time Slider                                                        | In both one-visible-layer cases the same custom slider remains exactly once in Available, never Hidden; its selected slider stays open. There are no separate entries for the three additional backing paths                                                                                                                                                                                                           | M    |
| Custom slider all hidden and eye restores all                | All four backing paths control one Hidden entry                    | 1. On Map 11, select the custom slider; hide its main path and two additional paths, leaving historical-flood/0 on<br>2. Hide historical-flood/0 last; also hide Airborne chart group as an unrelated control<br>3. Reopen Time Slider, inspect the lists, then click the custom slider's inline eye once<br>4. Inspect Layers and Time Slider again                                                                          | Only after the last backing layer is hidden does the custom entry leave Available and appear once in Hidden; selection clears to the guide. One eye click turns on ALL four paths, not just fire_progression/0, returns one entry to Available, and selects its slider. Airborne chart group remains off; time-filter settings do not change                                                                           | M    |
| Parent-hidden child eye restores ancestor visibility         | Group visibility is restored from each consumer panel              | 1. On Map 11, complete setup separately for each of Details, Data Table, and Chart<br>2. Hide Airborne chart group only; leave its child Airborne stations individually on<br>3. Open the consumer panel and inspect the child's Hidden entry<br>4. Click the child's inline eye and verify Layers, map rendering, and that consumer's Available list<br>5. Reset and repeat for the other two panels                         | The parent-hidden child remains listed under Hidden in each consumer. Its eye enables Airborne chart group as well as the child, the stations render again, and the child returns once to Available with its content selected. Merely setting the already-on child without enabling its ancestor is a failure                                                                                                          | M    |
| Hidden-list semantics and keyboard eye                       | Real list semantics, non-interactive rows, and focus after remount | 1. On Map 11, repeat a hide/show cycle in Details, Data Table, Chart, and Time Slider (hide all four backing paths for the slider)<br>2. Inspect each panel's heading/list associations and hidden row; click the row text without using its eye<br>3. Enter keyboard navigation and Tab to the eye; activate with Enter, then repeat using Space<br>4. Observe focus and the screen reader announcement while the item moves | Each panel uses distinct labelled Available/Hidden native lists, not a single visual grouping. Hidden row has tabIndex=-1 and is skipped by Tab; text click does nothing; the enabled eye has an accessible Show-layer label. Enter/Space restores the layer(s), announces the move through the polite live region, and focus follows selection into the right panel rather than being lost to the page or another map | M    |
...
| WMS duplicate group names (#3521) | Duplicate nested `<Name>` in tree | 1. Paste `https://maps.geogratis.gc.ca/wms/canimage_en?SERVICE=WMS&REQUEST=GetCapabilities&VERSION=1.3.0`<br>2. Wait for the layer tree to populate<br>3. Expand and select the `canimage` group checkbox | Tree expands and the group selects without crashing (no `RangeError: Maximum call stack size exceeded`); viewer stays responsive | M    |
...
### WMS Duplicate Group Names (#3521)

Config: inline on Map 10 — WMS `canimage_en` where a parent group and its child group share the same `<Name>` (`canimage`), configured with `layerId: 'canimage'`. The OGC WMS spec allows duplicate names across nesting levels.

| Test                         | Description                    | Steps                                                                      | Expected Result                                                                                                           | Auto |
| ---------------------------- | ------------------------------ | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ---- |
| Config loads duplicate group | No infinite loop on config add | 1. On Map 10, wait for the map to finish loading                           | Map loads without hanging; the `canimage` group and its sublayers (e.g. `canimage-030`) appear in the legend/layers panel | M    |
| Duplicate group renders      | Sublayers render on the map    | 1. On Map 10, ensure the `canimage` group is visible<br>2. Observe the map | The CanImage tiles render on the map                                                                                      | M    |

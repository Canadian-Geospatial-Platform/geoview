...
- Standardized theme spacing to MUI's default 8px scale: removed the custom `spacing()` override, converted spacing values to explicit `theme.spacing()` calls, and rounded off-grid values to the nearest standard factor; enabled MUI's `cssVariables` (CSS custom properties). Also removed the redundant `itemsGrid &:first-of-type` borders in `layer-details-style.ts` (a `style.css` rule was overriding it so the borders never actually showed), fixed the opacity slider label wrapping onto two lines when disabled, migrated most inline CSS to dedicated CSS files, and removed/optimized redundant CSS across packages (#3069)

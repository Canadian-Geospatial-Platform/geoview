...
Orientation is not programmatically locked and the application passes 1.3.4. Any layout limitations when using small viewports and/or magnified) are captured under 1.4.10 Reflow.
...
This barely passes (low contrast). WCAG 2.2 is more strict about this and implementing higher contsrast would be a good future enhancement.
...
Any further keyboard navigation improvements beyond this UX enhancements and would be assessed under other success criteria (e.g. 2.1.1 Keyboard) on a case-by-case basis. They do not constitute a failure of 2.4.1.
...
## Pass Explanation(s)s

### 2.1.1 - Keyboard

**Snackbar behaviour:**
The snackbar’s autoHideDuration is set to **5000ms**. Messages disappears after that amount of time. Whether or not this is enough time for a to read messages depends on user, and app context.
...
Certain charts will be less useable on smaller viewports, when zoomed in la lot (labels might overlap, the chart will be too small to be useful, etc,
...
- Although this can slightly confusing visually when expecting top-bottom / left-right navigation, logically it makes sense for the second thumb to receive focus first (the bottom thumb represents the minimum value)
...
- Providing meaningful alt text for the icons in the table is not programatically feasible. alt=”” is being used.

### 1.3.3 - Sensory Characteristics

- The icons within the table rely on shape/colour in order to create a visual relationship to what’s seen on he map. However, the data table provides sufficient context for users to understand the data, without users needing to interpret the icons.

...
On first tab into the map, users are prompted to enable keyboard navigation mode, which enables a focus trap within the map, and activates crosshair-based map interaction. Press Ctrl + Q to exit.
...
- Close the active panel and restores focus to the triggering button, or
- Closes modal dialogs (lightbox, export, data table)

...
Clickable `<div>` and `<span>` elements are inaccessible by default — they receive no keyboard focus, emit no semantic role, and are invisible to assistive technologies. Native `<button>` and `<a>` elements come with built-in keyboard support, appropriate ARIA roles, and browser-managed focus behavior at no extra cost. In MUI, use Button, IconButton, and Link components over attaching onClick handlers to arbitrary elements.
...
### 7- Focus management: Do not use disabled on UI elements that get removed from the DOM

Some UI elements should not be removed from the UI after they are pressed.

- Use aria-disabled instead of disabled
- Style the the aria-disabled element to look like it would if disabled

### 8- Avoid removing buttons from the DOM
...
### 11- Provide screen reader version of Loading indicators
...
See SC 1.3.1 — Known limitation. Fixing that issue would resolve this limitation as well.

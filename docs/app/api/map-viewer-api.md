...
- [onMapLayersProcessed](#onmaplayersprocessed)
- [onMapLayersLoaded](#onmaplayersloaded)
- [onMapMoveEnd](#onmapmoveend)
- [onMapPointerMove](#onmappointermove)
- [onMapPointerStop](#onmappointerstop)
- [onMapSingleClick](#onmapsingleclick)
- [onMapZoomEnd](#onmapzoomend)
- [onMapRotation](#onmaprotation)
- [onMapChangeSize](#onmapchangesize)
- [onMapProjectionChanged](#onmapprojectionchanged)
- [onMapComponentAdded](#onmapcomponentadded)
- [onMapComponentRemoved](#onmapcomponentremoved)
 ...
 */
const drawInteraction = mapViewer.initDrawInteractions(
  "my-geometries",
  "Polygon",
  {
    strokeColor: "blue",
 ...
 */
const lonLatCoord = mapViewer.convertCoordinateMapProjToLonLat([
  2673031.1037, 8438703.5471,
]);
```
 ...
 */
const lonLatExtent = mapViewer.convertExtentMapProjToLonLat([
  2673031.1037, 8438703.5471, 2673031.1037, 8438703.5471,
]);
```
...
<a id="onmaplayersprocessed"></a>

#### onMapLayersProcessed

```typescript
/**
 * Registers a map layers processed event callback.
 * Fired when all layers have reached the 'processed' status.
 *
...
mapViewer.onMapLayersProcessed((sender) => {
  console.log("All layers processed for map:", sender.mapId);
});
```

<a id="onmaplayersloaded"></a>
 ...
 * Fired when all layers have reached the 'loaded' status.
 *
...
mapViewer.onMapLayersLoaded((sender) => {
  console.log("All layers loaded for map:", sender.mapId);
});
```
 ...
 * Fired when the mouse/pointer moves over the map.
 *
...
mapViewer.onMapPointerMove((sender, payload) => {
  console.log("Pointer moved to:", payload.lonlat, "Pixel:", payload.pixel);
});
```

<a id="onmappointerstop"></a>
 ...
 * Fired when the mouse/pointer stops moving over the map.
 *
 * @param {MapPointerMoveDelegate} callback - The callback to be executed whenever the event is emitted
 */
...
mapViewer.onMapSingleClick((sender, payload) => {
  console.log("Map clicked at:", payload.lonlat, "Pixel:", payload.pixel);
});
```
 ...
 * Fired when the map rotation changes.
 *
...
mapViewer.onMapRotation((sender, payload) => {
  console.log("Map rotation changed to:", payload.rotation);
});
```

<a id="onmapchangesize"></a>
 ...
 * Fired when the map size/dimensions change.
 *
...
mapViewer.onMapChangeSize((sender, payload) => {
  console.log("Map size changed to:", payload.size);
});
```
 ...
 * Fired when the map projection is changed.
 *
...
mapViewer.onMapProjectionChanged((sender, payload) => {
  console.log("Map projection changed to:", payload.projection.getCode());
});
```

<a id="onmapcomponentadded"></a>
 ...
 * Fired when a custom component is added to the map.
 *
...
<a id="onmapcomponentremoved"></a>

#### onMapComponentRemoved

```typescript
/**
 * Registers a component removed event callback.
 * Fired when a custom component is removed from the map.
 *

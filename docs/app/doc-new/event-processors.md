...
- ✅ Manage plugin state (time slider, geochart)
...
**Purpose:** Manages the swiper plugin for layer comparison.
...
**Purpose:** Manages the drawer plugin for geometry drawing and editing.
...
Plugin Event Processors may not be initialized if the plugin isn't loaded:

```typescript
import { TimeSliderEventProcessor } from "geoview-core";

// Always check plugin initialization
if (TimeSliderEventProcessor.isTimeSliderInitialized("mapId")) {
  const timeLayers = TimeSliderEventProcessor.getTimeSliderLayers("mapId");
} else {
...
### Pattern 3: Querying Plugin State

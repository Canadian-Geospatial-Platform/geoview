...
| Junction highlight matches details (ESRI Dynamic) | Highlight pairs to the shown feature at shared boundaries | 1. On Map 1, click exactly on a shared boundary/junction of the GNWT Boundaries layer where several polygons meet, so identify returns multiple features<br>2. In Details, step through the returned features with the prev/next arrows | The highlighted polygon always matches the feature whose attributes are shown (each navigation index), not a neighbouring polygon (#3636)            | A    |

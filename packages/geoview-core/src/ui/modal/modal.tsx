...
export function Modal(props: TypeDialogProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    api.event.on(
      ...
      (args) => {
        if (id === (args.id as TypeJSONValue as string) && (args.handlerName as TypeJSONValue as string) === mapId) {
          const modal = api.map(mapId).modal.modals[args.id as TypeJSONValue as string] as TypeModalProps;

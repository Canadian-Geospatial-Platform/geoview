  ...
  useEffect(() => {
    ...
    if (open) {
      ...
      delayJob.promise
        ...
        .catch((error) => {
          // Delay was cancelled or rejected - this is expected during cleanup
          logger.logDebug('Panel open delay cancelled:', error);
        });
    } else {
      // Wait the transition period (+50 ms just to be sure of shenanigans)
      delayJob = doTimeout(theme.transitions.duration.standard + 50);
      ...
      delayJob.promise
        ...
        .catch((error) => {
          logger.logDebug('Panel close delay cancelled:', error);
        });
    }

...
export const generateThemeOptions = (geoViewColors: IGeoViewColors = defaultGeoViewColors): ThemeOptions => {
  ...
  const themeOptions: ThemeOptions = {
    ...
    components: {
      MuiCssBaseline: {
        styleOverrides: `
          /* WCAG-compliant focus indicators for standard HTML interactive elements */
          /* Scoped to avoid conflicts with custom focus handling (map container, data tables, etc.) */
          /* Note: MUI components require component-level overrides due to higher CSS specificity */
          
    button:not([class*="Mui"]):focus-visible,
    a:not([class*="Mui"]):focus-visible,
    input:not([class*="Mui"]):focus-visible,      /* native form inputs */
    select:not([class*="Mui"]):focus-visible,     /* native selects */
    textarea:not([class*="Mui"]):focus-visible {  /* native textareas */
      outline: ${FOCUS_OUTLINE_WIDTH} solid ${geoViewColors.focusIndicator.outline};
      outline-offset: ${FOCUS_OUTLINE_OFFSET};
      box-shadow: 0 0 0 ${FOCUS_HALO_SIZE} ${geoViewColors.focusIndicator.halo};
    }
        `,
      },
      MuiTooltip: {

...
/**
 * Handles when the div containing the MapViewer disappears from the DOM, likely by another dev manipulation.
 * @param {string} mapId - The MapId of the MapViewer which had its div disappear from the DOM.
 */
function handleDivDisappeared(mapId: string): void {
  ...
  if (api.hasMapViewer(mapId)) {
    ...
    api.deleteMapViewer(mapId, false).catch((error: unknown) => {
      // Log
      logger.logPromiseFailed('in deleteMapViewer in handleDivDisappeared', error);
    });
  }
}
...
async function renderMap(mapElement: Element): Promise<MapViewer> {
  ...
  // Set up MutationObserver to watch for removal of the div where our MapViewer is mounted
  const observer = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
      mutation.removedNodes.forEach((node) => {
        if (node === mapElement) {
          // The div disappeared, handle it
          handleDivDisappeared(mapId);

          // Stop watching
          observers[mapId].disconnect();
        }
      });
    });
  });

  // Watch the parent of your root div
  const parent = mapElement.parentNode;
  if (parent) {
    // Disconnect prior one if any
    if (observers[mapId]) observers[mapId].disconnect();

    // Keep the observer reference
    observers[mapId] = observer;

    // Observe the parent DOM node

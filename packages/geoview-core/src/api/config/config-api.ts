...
export class ConfigApi {
  ...
  static async createMapConfig(mapConfig: string | TypeJsonObject, language: TypeDisplayLanguage): Promise<MapFeatureConfig> {
    ...
    try {
      ...
      if (geocoreErrorDetected) mapFeatureConfig.raiseErrorDetectedFlag(); // If a geocore error was detected, signal it.
      // Start reading the metadata
      mapFeatureConfig.fetchAllServiceMetadata();
      ...
      defaultMapConfig.raiseErrorDetectedFlag();
  ...
  static async createLayerConfig(
    ...
    // If we have a valid layer configuration, we start reading the metadata.
    if (!ConfigApi.lastLayerConfigCreated?.errorDetected) ConfigApi.lastLayerConfigCreated?.fetchServiceMetadata();

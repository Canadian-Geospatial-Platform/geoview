...
export class ConfigApi {
  ...
  static async createInitConfigFromType(
    ...
    if (layerType === 'geoCore') {
      ...
      // Keep original metadataAccessPath from GeoCore if it has changed, WMS case where legend is not available from the layer url
      if (layerConfigForGeoview.metadataAccessPath !== layerConfigFromGeocore.metadataAccessPath)
        layerConfigForGeoview.metadataAccessPath = layerConfigFromGeocore.metadataAccessPath;
    ...
    switch (layerType) {
      ...
      case 'ogcWms':
        return WMS.initGeoviewLayerConfig(geoviewLayerId, geoviewLayerName, layerURL, true, isTimeAware);
      case 'xyzTiles':
  ...
  static processLayerFromType(
    ...
    switch (layerType) {
      ...
      case 'ogcWms':
        return WMS.processGeoviewLayerConfig(geoviewLayerId, geoviewLayerName, layerURL, layerIds as number[], false, true);
      case 'xyzTiles':

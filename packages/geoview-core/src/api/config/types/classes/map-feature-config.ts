...
export class MapFeatureConfig {
  ...
  constructor(providedMapFeatureConfig: string | TypeJsonObject, language: TypeDisplayLanguage) {
    ...
    this.map = Cast<TypeMapConfig>(defaultsDeep(gvMap, this.#getDefaultMapConfig(gvMap?.viewSettings?.projection as TypeValidMapProjectionCodes)));
  ...
  #getDefaultMapConfig(projection?: TypeValidMapProjectionCodes): TypeMapConfig {
    const proj = projection && VALID_PROJECTION_CODES.includes(projection) ? projection : CV_DEFAULT_MAP_FEATURE_CONFIG.map.viewSettings.projection;

...
export class WfsLayerEntryConfig extends AbstractBaseLayerEntryConfig {
  ...
  override async fetchLayerMetadata(): Promise<void> {
    ...
    const fromGetCapabilities = (this.getGeoviewLayerConfig() as WfsLayerConfig).findLayerMetadataEntry(this.layerId);
  ...
  #layerIsQueryable(): boolean {
    const serviceMetadata = this.getGeoviewLayerConfig().getServiceMetadata().fromGetCapabilities;
  ...
  async #fetchDescribeFeatureType(): Promise<TypeJsonObject[]> {
    ...
    const describeFeatureUrl = `${(this.getGeoviewLayerConfig() as WfsLayerConfig).processUrlParameters(
      'DescribeFeatureType'

...
export class GeoJsonLayerEntryConfig extends AbstractBaseLayerEntryConfig {
  ...
  constructor(layerConfig: TypeJsonObject, geoviewLayerConfig: AbstractGeoviewLayerConfig, parentNode?: EntryConfigBaseClass) {
    ...
    if (!this.source.dataAccessPath && !this.source.geojson) this.source.dataAccessPath = geoviewLayerConfig.metadataAccessPath;

    // Ensure that any passed geoJSON is passed to the new source.
    if (layerConfig.source?.geojson) this.source.geojson = layerConfig.source.geojson as string;
  }

...
export class XyzLayerConfig extends AbstractGeoviewLayerConfig {
  ...
  override async fetchServiceMetadata(): Promise<void> {
    const metadataUrlSplit = this.metadataAccessPath.toLowerCase().split('mapserver');
    let metadataUrl = metadataUrlSplit.length ? `${metadataUrlSplit[0]}mapserver` : this.metadataAccessPath;

    if (!metadataUrl.toLowerCase().endsWith('.meta') && !metadataUrl.toLowerCase().endsWith('json'))
      metadataUrl = metadataUrl.endsWith('/') ? `${metadataUrl}?f=json` : `${metadataUrl}/?f=json`;
    try {

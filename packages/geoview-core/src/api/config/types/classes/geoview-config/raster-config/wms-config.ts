...
export class WmsLayerConfig extends AbstractGeoviewLayerConfig {
  ...
  constructor(geoviewLayerConfig: TypeJsonObject, language: TypeDisplayLanguage) {
    ...
    const metadataAccessPathItems = this.metadataAccessPath.split('?Layers=');
    ...
    if (lastPathItem) {
      ...
      if (this.listOfLayerEntryConfig.length) {
        ...
        logger.logError('When an WMS metadataAccessPath ends with a layer index, the listOfLayerEntryConfig must be  empty.');
      }

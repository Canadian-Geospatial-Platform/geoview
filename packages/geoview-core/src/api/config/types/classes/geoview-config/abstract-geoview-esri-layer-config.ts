...
export abstract class AbstractGeoviewEsriLayerConfig extends AbstractGeoviewLayerConfig {
   ...
   * @private
   */
  #propagateErrorToAllLayers(listOfLayerEntryConfig: EntryConfigBaseClass[]): void {
    listOfLayerEntryConfig.forEach((layerEntry) => {
      layerEntry.raiseErrorDetectedFlag();
      if (layerEntryIsGroupLayer(layerEntry)) this.#propagateErrorToAllLayers(layerEntry.listOfLayerEntryConfig);
    });
  }
   ...
   * @private
   */
  async #fetchEsriMetadata(serviceUrl: string): Promise<TypeJsonObject> {
    try {
    ...
    this.#propagateErrorToAllLayers(this.listOfLayerEntryConfig);
    return {};
  }
   ...
   */
  override fetchServiceMetadata(): void {
    this.promiseLayerProcessed = new Promise<void>((resolve, reject) => {
      ...
      this.#fetchEsriMetadata(serviceUrl)
        .then((metadata) => {
  ...
  async fetchEsriLayerMetadata(subLayerConfig: AbstractBaseLayerEntryConfig): Promise<TypeJsonObject> {
    ...
    subLayerConfig.raiseErrorDetectedFlag();
    return {};

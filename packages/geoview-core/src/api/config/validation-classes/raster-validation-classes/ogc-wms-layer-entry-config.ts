...
export class OgcWmsLayerEntryConfig extends AbstractBaseLayerEntryConfig {
  ...
  #normalizeMetadataAndDataAccessPaths(): void {
    ...
    let metadataAccessPath = this.getMetadataAccessPath()!.toLowerCase().split('?request=getcapabilities')[0];
    ...
    let dataAccessPath = this.getDataAccessPath().toLowerCase().split('?request=getcapabilities')[0];

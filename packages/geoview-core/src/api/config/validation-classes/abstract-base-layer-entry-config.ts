...
export abstract class AbstractBaseLayerEntryConfig extends ConfigBaseClass {
  ...
  /**
   * Verifies and corrects the layer's `dataAccessPath` based on metadata rules.
   * This method resolves a scenario where the `dataAccessPath` stored in the layer configuration
   * (`layerConfig.source.dataAccessPath`) was automatically generated by the configuration validator
   * because the original configuration did not provide a value.
   * When this happens, the auto-generated `dataAccessPath` is usually the **root of the metadata
   * access path**, with the layer ID appended. However, if the metadata itself provides a valid
   * `dataAccessPath`, that value should take precedence.
   * The method applies the following logic:
   * 1. If the metadata does **not** provide a `dataAccessPath`, nothing is changed.
   * 2. The method extracts the parent directory of the metadata access path.
   * 3. It reconstructs a derived path: `<metadataRoot>/<layerId>`.
   * 4. If this derived path matches the current `source.dataAccessPath`, it is assumed to be a
   *    fallback auto-generated path, and therefore the method replaces it with the authoritative
   *    `dataAccessPath` stored in the metadata.
   * @param {TypeBaseSourceInitialConfig} metadataSource - The metadata object that may contain an authoritative
   *   `dataAccessPath` for the layer.
   */
  verifyDataAccessPath(metadataSource: TypeBaseSourceInitialConfig | undefined): void {
    ...
    // Check the logic
    let metadataAccessPathRoot = this.getMetadataAccessPath();
    if (metadataAccessPathRoot) {
      ...
      const metadataAccessPathRootPlusLayerId = `${metadataAccessPathRoot}/${this.layerId}`;
      if (metadataAccessPathRootPlusLayerId === this.source?.dataAccessPath) {
        // Favor the dataAccessPath of the metadata source over the automatic dataAccessPath
        // debugger;

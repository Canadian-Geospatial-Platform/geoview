...
export class TimeSliderEventProcessor extends AbstractEventProcessor {
  ...
  static getInitialTimeSliderValues(mapId: string, layerPath: string): TimeSliderLayerSet {
    ...
    const name = getLocalizedValue(layerConfig.layerName, MapEventProcessor.getDisplayLanguage(mapId)) || layerConfig.layerId;
    ...
    if (featureInfo) {
      ...
      localizedOutFields = getLocalizedValue(outfields, MapEventProcessor.getDisplayLanguage(mapId))?.split(',');
      localizedAliasFields = getLocalizedValue(aliasFields, MapEventProcessor.getDisplayLanguage(mapId))?.split(',');
    }

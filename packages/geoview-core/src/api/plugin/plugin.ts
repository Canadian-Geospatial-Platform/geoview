...
export class Plugin {
  ...
  loadScript = (pluginId: string): Promise<any> => {
    return new Promise((resolve) => {
      ...
      if (!existingScript) {
        ...
        whenThisThen(() => window.geoviewPlugins && window.geoviewPlugins[pluginId]).then(() => {
          resolve(window.geoviewPlugins[pluginId]);
        });
      }
    });
  ...
  addPlugin = async (
    ...
    if (api.maps[mapId] && !api.maps[mapId].plugins[pluginId]) {
      ...
      if (plugin) {
        ...
        // attacht he plugin tot he map plugins object

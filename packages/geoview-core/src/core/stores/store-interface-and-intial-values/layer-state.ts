  ...
  const init = {
    ...
    actions: {
      ...
      getLayerBounds: (layerPath: string) => {
        const layer = api.maps[get().mapId].layer.getGeoviewLayer(layerPath);
        if (layer) {
      ...
      toggleItemVisibility: (layerPath: string, geometryType: TypeStyleGeometry, itemName: string) => {
        ...
        if (layer) {
          ...
          (api.maps[get().mapId].layer.getGeoviewLayer(layerPath) as AbstractGeoViewVector).applyViewFilter(layerPath, '');
        }
        set({
          layerState: {
            ...get().layerState,
...
function setOpacityInLayerAndChildren(layer: TypeLegendLayer, opacity: number, mapId: string, isChild = false) {
  ...
  api.maps[mapId].layer.getGeoviewLayer(layer.layerPath).setOpacity(opacity, layer.layerPath);
  if (isChild) {

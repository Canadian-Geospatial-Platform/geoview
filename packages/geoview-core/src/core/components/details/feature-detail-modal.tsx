...
export default function FeatureDetailModal(): JSX.Element {
  ...
  // Determine which panel (appBar or footerBar) the data table is in
  ...
  return (
    <Modal
      ...
      onClose={() => {
        ...
        const lightboxOpen = document.querySelector('.yarl__container');
        if (!lightboxOpen) {
          disableFocusTrap();
        }
      }}
      ...
      contentModal={
        <>
          ...
          <List sx={sxClasses.featureDetailListContainer}>
            <FeatureInfoTable featureInfoList={featureInfoList} containerType={containerType} />
          </List>
        </>
      }
    />

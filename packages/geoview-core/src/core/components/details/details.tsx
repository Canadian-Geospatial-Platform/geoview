...
import { LayersList as LayerListFooter } from './layers-list-footer';
 ...
 * @returns {JSX.Element} returns the Details component
 */
export function DetailsFooter(props: TypeDetailsProps): JSX.Element | null {
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const { arrayOfLayerData, detailsSettings } = props;
  const { location, handlerName } = detailsSettings;
  const [details, setDetails] = useState<TypeArrayOfLayerData>([]);
  ...
  return <LayerListFooter arrayOfLayerData={details} detailsSettings={detailsSettings} />;
}

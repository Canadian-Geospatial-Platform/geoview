...
function LegendContainerComponent({ layers }: LegendContainerProps): JSX.Element {
  ...
  const renderLayer = (layer: TypeLegendLayer): JSX.Element => {
    ...
    return (
      <div key={layer.layerPath} style={getSxClasses(theme).legendSpacing}>
        {/* Layer icon and name */}
        <div style={getSxClasses(theme).legendTitle}>
          <span>{layer.layerName}</span>
        </div>
        <div>
          <span style={getSxClasses(theme).toLine}>{renderWMSLayerImage(layer, 'icon')}</span>
        </div>
        {/* Children */}
        ...
        {layer.items && layer.items.length > 0 && (
          <div style={getSxClasses(theme).toLine}>

        ...
        <ListItemText
          primary={
            <Typography component="div" id={layerNameId}>
              {layerName}
              {(layerHidden || layerStatus === 'error') && (
                <Box component="span" sx={sxClasses.visuallyHidden} className="srOnly">
                  {' '}
                  {t('legend.layerOutOfRangeScreenReader')}
                </Box>
              )}
            </Typography>
          }
 ...
 */
export function LegendLayer({ layerPath, showControls, mapId, containerType }: LegendLayerProps): JSX.Element {
  // Log

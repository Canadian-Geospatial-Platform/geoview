...
export class NavbarButtons {
  ...
  private createButtonPanel = (
    ...
    if (buttonProps) {
      ...
      const buttonPanelId = generateId(buttonProps.buttonPanelId);
      ...
      if (panelProps) {
        ...
        buttonPanel.panel = new PanelApi(panel, buttonPanelId, this.mapId);
      }

      // add the new button panel to the correct group
      this.buttons[group][buttonPanelId] = buttonPanel;

      // trigger an event that a new button or button panel has been created to update the state and re-render
      api.event.emit(
        buttonPanelPayload(EVENT_NAMES.NAVBAR.EVENT_NAVBAR_BUTTON_PANEL_CREATE, this.mapId, buttonPanelId, group, buttonPanel)
      );

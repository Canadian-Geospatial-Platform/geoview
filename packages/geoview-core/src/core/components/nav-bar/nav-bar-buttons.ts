...
export class NavbarButtons {
  ...
  private createButtonPanel = (
    ...
    if (buttonProps) {
      ...
      const theButtonId = generateId(buttonProps.id);
      ...
      if (panelProps) {
        ...
        buttonPanel.panel = new PanelApi(panel, theButtonId, this.mapId);
      }

      // add the new button panel to the correct group
      if (group !== '__proto__' && theButtonId !== '__proto__') this.buttons[group][theButtonId] = buttonPanel;

      // trigger an event that a new button or button panel has been created to update the state and re-render
      api.event.emit(buttonPanelPayload(EVENT_NAMES.NAVBAR.EVENT_NAVBAR_BUTTON_PANEL_CREATE, this.mapId, theButtonId, group, buttonPanel));

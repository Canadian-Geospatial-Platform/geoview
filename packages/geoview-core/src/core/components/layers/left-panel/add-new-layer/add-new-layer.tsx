...
export function AddNewLayer(): JSX.Element {
  ...
  const handleStep1 = (): void => {
    ...
    const guessedLayerType = api.config.guessLayerType(layerURL);
      ...
      try {
        ...
        if (geoviewLayerConfig && !geoviewLayerConfig.getErrorDetectedFlag()) {
          ...
          if (
            ...
            (geoviewLayerConfig.geoviewLayerName === 'inconnue' && (metadata.mapName || metadata.Service?.Name))
          )
            geoviewLayerConfig.geoviewLayerName = (metadata.mapName as string) || (metadata.Service?.Name as string);

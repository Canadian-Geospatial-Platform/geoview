 ...
 */
export const buildGeoLayerToAdd = (inputProps: BuildGeoViewLayerInput): TypeGeoviewLayerConfig | undefined => {
  ...
  if (layersToAdd.length) {
     ...
     */
    const allSubLayersAreIncluded = (groupLayer: GroupLayerEntryConfig): boolean => {
      return groupLayer.listOfLayerEntryConfig.every((layerEntryConfig) =>
        !layerEntryConfig.isLayerGroup
          ...
          : layerIds.includes(layerEntryConfig.layerId) && allSubLayersAreIncluded(layerEntryConfig as GroupLayerEntryConfig)
      );
    };
    ...
    const createLayerEntryConfigForGroupLayer = (groupLayer: GroupLayerEntryConfig): LayerEntryConfigShell => {
      ...
      if (allSubLayersAreIncluded(groupLayer) && layerType === CV_CONST_LAYER_TYPES.ESRI_DYNAMIC) {
        return {

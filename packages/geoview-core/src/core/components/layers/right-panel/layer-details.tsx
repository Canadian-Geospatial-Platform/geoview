...
export function LayerDetails(props: LayerDetailsProps): JSX.Element {
  ...
  const handleOpenTable = () => {
    openModal({ activeElementId: 'layerDatatable', callbackElementId: `test` });
  };
  ...
  function renderLayerButtons() {
    return (
      <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '15px' }}>
        <IconButton id="test" tooltip="legend.tableDetails" sx={{ backgroundColor: '#F6F6F6' }} onClick={handleOpenTable}>
          <TableViewIcon />
        </IconButton>

...
function MapDataTable({ data, layerId, mapId, layerKey }: MapDataTableProps) {
  ...
  const filterMap = debounce((filters: MRTColumnFiltersState) => {
    ...
    // TODO: use Store
    const geoviewLayerInstance = api.maps[mapId].layer.geoviewInstance(layerKey);
    ...
    if (mapFilteredRecord[layerKey] && geoviewLayerInstance !== undefined && filterLayerConfig !== undefined && filterStrings.length) {
      (api.maps[mapId].layer.geoviewInstance(layerKey) as AbstractGeoViewVector | EsriDynamic)?.applyViewFilter('', filterStrings);
    } else {
      (api.maps[mapId].layer.geoviewInstance(layerKey) as AbstractGeoViewVector | EsriDynamic)?.applyViewFilter();
    }
  }, FILTER_MAP_DELAY);

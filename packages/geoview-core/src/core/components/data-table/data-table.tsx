...
function DataTable({ data, layerPath, tableHeight = '500px' }: DataTableProps): JSX.Element {
  ...
  const { zoomToExtent, highlightBBox, transformPoints, showClickMarker, addHighlightedFeature, removeHighlightedFeature } =
    useMapStoreActions();
  const { applyMapFilters, getESRIDynamicFeatureExtent, setSelectedFeature } = useDataTableStoreActions();
  ...
  const handleZoomIn = useCallback(
    async (feature: TypeFeatureInfoEntry) => {
      let { extent } = feature;
      if (feature.geoviewLayerType === CONST_LAYER_TYPES.ESRI_DYNAMIC && !extent) {
        extent = await getESRIDynamicFeatureExtent(layerPath, feature);
      }
      if (extent) {
        // Project

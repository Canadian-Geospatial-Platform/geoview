 ...
 * Tooltip is shown only when content is actually truncated (scrollWidth > offsetWidth).
 *
...
function TooltipCell({ children, title, isOpen = false }: TooltipCellProps): JSX.Element {
  ...
  /**
   * Checks if content is overflowing its container.
   *
   * @returns void
   */
  ...
  /**
   * Detects content overflow on mount and value changes.
   */
  useEffect(() => {
    logger.logTraceUseEffect('TOOLTIP-CELL - checkOverflow on mount/value change', title);
    ...
    // NOTE: checkOverflow intentionally omitted — stable reference with empty deps
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [title]);

  /**
   * Observes resize events to re-check overflow when column width changes.
   */
  useEffect(() => {
    logger.logTraceUseEffect('TOOLTIP-CELL - ResizeObserver setup');
      ...
      columnList.push({
        ...
        Cell: ({ cell }) =>
          getCellValueWithTooltip(
            cell.getValue() as string | number | JSX.Element,
            cell.id,
            cell.row.original.gvFeature as unknown as TypeFeatureInfoEntry
          ),

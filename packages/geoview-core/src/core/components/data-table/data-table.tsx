...
/**
 * Creates a minimal external store for tracking focused cell/header IDs.
 *
 * Exists so focus changes don't force memoColumns to recompute. Consumers
 * subscribe via useSyncExternalStore and only re-render when the specific
 * ID they care about changes state, not on every focus event in the table.
 *
 * @returns FocusStore object with setFocused, subscribe, and isFocused methods
 */
...
/** Type representing the focus store instance. */
type FocusStore = ReturnType<typeof createFocusStore>;

/** No-op fallback store — used only if FocusAwareTooltipCell somehow renders outside the Provider. */
const FALLBACK_FOCUS_STORE = createFocusStore();

/**
 * Subscribes to whether a specific cell/header ID is currently focused.
 *
 * Only triggers a re-render when this particular ID's focus state flips.
 *
 * @param store - The focus store instance for this table
 * @param id - The cell or header ID to track (e.g. cell.id or `header-${columnId}`)
 * @returns Whether the given ID currently has focus
 */
function useIsCellFocused(store: FocusStore, id: string): boolean {
  return useSyncExternalStore(store.subscribe, () => store.isFocused(id));
}
...
function FocusAwareTooltipCell({ cellId, title, children }: FocusAwareTooltipCellProps): JSX.Element {
  ...
  const isOpen = useIsCellFocused(store ?? FALLBACK_FOCUS_STORE, cellId);
...
function DataTable({ data, layerPath, containerType, unfilteredFeaturesCount }: DataTableProps): JSX.Element {
  ...
  const focusStoreRef = useRef<FocusStore>(createFocusStore());
  ...
  const handleCellBlur = useCallback((): void => {
    focusStoreRef.current.setFocused(null);
  }, []);
    ...
    (cellValue: string | number, cellId: string, feature?: TypeFeatureInfoEntry): string | number | JSX.Element => {
      ...
      if ((typeof cellValue === 'string' && cellValue.length) || typeof cellValue === 'number') {
        try {
          ...
          // If content is unchanged (no HTML tags added), return as plain text
          if (sanitized === cellValue.toString()) {
            return cellValue;
          }

          // Otherwise, parse HTML content through UseHtmlToReact with omitWrappers to avoid redundant divs
          const reactComponent = <UseHtmlToReact htmlContent={sanitized} omitWrappers />;
  ...
  const handleCellFocus = useCallback(
    (event: React.FocusEvent<HTMLTableCellElement>): void => {
      ...
      if (cellId) focusStoreRef.current.setFocused(cellId);
  ...
  const handleTableHeadCellFocus = useCallback(
    (event: React.FocusEvent<HTMLTableCellElement>): void => {
      ...
      if (columnId) {
        focusStoreRef.current.setFocused(`header-${columnId}`);
      }
      // Defer scroll correction to avoid layout thrashing during rapid navigation
      requestAnimationFrame(() => scrollCellIntoView(cellEl));
    },
  ...
  const memoMuiTablePaperProps = useCallback(
    ({ table }: { table: MRTTableInstance<DataTableRow> }) => ({
      style: {
  ...
  const memoMuiTableBodyCellProps = useCallback(
    ({ cell }: { cell: MRTCell<DataTableRow> }) => ({
  ...
  const memoMuiTableHeadCellProps = useCallback(
    ({ column }: { column: MRTColumn<DataTableRow> }) => ({
      sx:

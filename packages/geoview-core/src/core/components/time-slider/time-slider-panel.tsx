...
import { SliderFilterProps } from './time-slider';
...
export function TimeSliderPanel(TimeSliderPanelProps: TimeSliderPanelProps) {
      ...
      label: timeframe
        ? `${
            timeframe === 'day'
              ...
              : new Date(timeMarks[i]).toLocaleDateString().slice(5)
          }`
        : new Date(timeMarks[i]).toLocaleDateString(),
    });
  }
  ...
  function valueLabelFormat(value: number): string {
    ...
    if (timeframe === 'year') return new Date(value).toLocaleDateString().slice(5);

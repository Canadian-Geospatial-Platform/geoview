  ...
  zoomToPointMarkersOrGroup(group: string, ids?: string[]): void {
    ...
    if (!ids) {
      groupMarkers.forEach((marker) => {
        idList.push(`${group}-${marker.id}`);
      });
    } else idList = ids.map((id) => `${group}-${id}`);
    ...
    if (extent)
      ...
      const errorMessageEnd = ids ? `, or point marker ids ${ids} are not correct.` : '.';
      logger.logError(`Point marker group ${group} has no markers or does not exist${errorMessageEnd}`);
    }
  }

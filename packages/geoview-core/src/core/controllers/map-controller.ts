...
import { InvalidExtentError, NoBoundsError } from '../exceptions/geoview-exceptions';
...
export class MapController extends AbstractMapViewerController {
  ...
  async setProjection(projectionCode: TypeValidMapProjectionCodes): Promise<void> {
    try {
      ...
      this.getControllersRegistry()
        ...
        .forEach((layer) => {
          // Remove the layer
          this.getMapViewer().layer.removeLayerUsingPath(layer.getLayerPath());
  ...
  addGeoviewLayer(geoviewLayerConfig: TypeGeoviewLayerConfig, abortSignal?: AbortSignal): GeoViewLayerAddedResult {
    // Redirect to layer api
    return this.getMapViewer().layer.addGeoviewLayer(geoviewLayerConfig, abortSignal);
  }
  ...
  replaceOrderedLayerInfo(layerConfig: ConfigBaseClass, layerPathToReplace?: string): void {
    ...
    // TODO: REFACTOR IMPORTANT - This function shouldn't be allowed to climb back on the LayerApi, redesign this..
  ...
  createGeometryGroup(groupName: string): void {
    const geometryApi = this.getMapViewer().layer.geometry;

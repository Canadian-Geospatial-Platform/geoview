...
export class LayerController extends AbstractMapViewerController {
   ...
   */
  constructor(mapViewer: MapViewer, layerDomain: LayerDomain, featureHighlight: FeatureHighlight) {
    super(mapViewer);

    // Keep the domain internally
    this.#layerDomain = layerDomain;

    // Keep the feature highlight internally
  ...
  protected override onUnhook(): void {
    ...
    // Listens when the layer first loaded state is changed in the Layer domain
  ...
  getExtentFromFeatures(layerPath: string, objectIds: number[], outfield?: string): Promise<Extent> {
    ...
    return layer.getExtentFromFeatures(objectIds, this.getMapViewer().getProjection(), outfield);
  }
  ...
  resetLayer(layerPath: string): Promise<void> {
    ...
    MapEventProcessor.setOrToggleMapLayerVisibility(this.getMapId(), layerPath, visibility);

    if (visibility) {
      // Return the promise that all items visibility will be renderered if layer is set to visible
      return LegendEventProcessor.setAllItemsVisibility(this.getMapId(), layerPath, visibility, true);
    }
  ...
  setHighlightLayer(layerPath: string): void {
    ...
    const highlightedLayerpath = MapEventProcessor.changeOrRemoveLayerHighlight(this.getMapId(), layerPath, currentHighlight);
  ...
  #handleLayerVisibleChanged(sender: LayerDomain, event: DomainLayerVisibleChangedEvent): void {
    // Save in the store (and do a bit more than that.. to review..)
    MapEventProcessor.setMapLayerVisibility(this.getMapId(), event.layer.getLayerPath(), event.visible);
  }

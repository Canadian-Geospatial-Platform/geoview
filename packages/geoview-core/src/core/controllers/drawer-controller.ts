...
import { getStoreDisplayLanguage } from '../stores/store-interface-and-intial-values/app-state';
import Collection from 'ol/Collection';
import { Projection } from '@/geo/utils/projection';
import { getArea, getLength } from 'ol/sphere';
import { getStoreMapCurrentProjection } from '../stores/store-interface-and-intial-values/map-state';
...
export class DrawerController extends AbstractMapViewerController {
  ...
  startEditing(): void {
    ...
    if (viewer.layer.geometry.hasGeometryGroup(DrawerController.DRAW_GROUP_KEY)) {
  ...
  clearDrawings(saveHistory: boolean = true): void {
    ...
    viewer.layer.geometry.deleteGeometriesFromGroup(DrawerController.DRAW_GROUP_KEY);
    ...
    reader.onload = (e) => {
      try {
        ...
        geojson.features.forEach((geoFeature: GeoJsonFeature) => {
          ...
          viewer.layer.geometry.addToGeometryGroup(feature, DrawerController.DRAW_GROUP_KEY);
  ...
  #deleteFeaturesAction(action: DrawerHistoryAction): void {
    ...
    action.features.forEach((feature) => {
      ...
      const geometryGroup = viewer.layer.geometry.getGeometryGroup(DrawerController.DRAW_GROUP_KEY);

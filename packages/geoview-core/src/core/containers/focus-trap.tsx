...
export function FocusTrapDialog(props: FocusTrapProps): JSX.Element {
    ...
    const focusTimeout = setTimeout(() => {
      ...
      if (topLink && !activeTrapGeoView && !hasBeenPromptedRef.current) {
        ...
        if (geoviewElement.contains(currentFocus)) {
          logger.logDebug('FOCUS-TRAP - Setting initial focus to toplink from:', currentFocus);
          topLink.focus();
  ...
  useEffect(() => {
    ...
    const handleTabNavigation = (evt: KeyboardEvent): void => {
      // Only intercept Tab key (both forward and backward)
      ...
      // If tabbing forward and not on toplink/bottomlink, redirect to toplink
      if (!evt.shiftKey && currentFocus !== topLink && currentFocus !== bottomLink) {
        logger.logDebug('FOCUS-TRAP - Intercepting forward Tab, redirecting to toplink from:', currentFocus);
        evt.preventDefault();
        topLink?.focus();
        return;
      }

      // If tabbing backward and not on toplink/bottomlink, redirect to bottomlink
      if (evt.shiftKey && currentFocus !== topLink && currentFocus !== bottomLink) {
        logger.logDebug('FOCUS-TRAP - Intercepting backward Tab, redirecting to bottomlink from:', currentFocus);
        evt.preventDefault();
        bottomLink?.focus();
        return;
      }

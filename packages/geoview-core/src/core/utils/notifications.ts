...
export class Notifications {
  ...
  #addSnackbarMessage(
    ...
    if (this.snackbarMessageQueue.length > 4 && !withNotification) {
      ...
      // For multiple slow render, fetch, or metadata warnings, replace all with a single multi layer message
      if (
        ...
        if (this.snackbarMessageQueue.length > 1)
      ...
      if (
        messageKey === 'warning.layer.slowFetch' &&
        this.snackbarMessageQueue.find(
          (snackbarMessage) =>
            snackbarMessage.messageKey === 'warning.layer.slowFetch' || snackbarMessage.messageKey === 'warning.layer.slowFetches'
        )
      ) {
        // Only replace messages in queue if there are more than one, otherwise the new message will be lost when snackbar closes
        if (this.snackbarMessageQueue.length > 1)
          this.snackbarMessageQueue = this.snackbarMessageQueue.filter(
            (snackbarMessage) => snackbarMessage.messageKey !== messageKey && snackbarMessage.messageKey !== 'warning.layer.slowFetches'
          );
        // eslint-disable-next-line no-param-reassign
        messageKey = 'warning.layer.slowFetches';
      }
      if (

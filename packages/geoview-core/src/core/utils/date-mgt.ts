...
export class DateMgt {
  ...
  #createAbsoluteInterval(ogcTimeDimension: string): string[] {
    ...
    // set min and max
    const min: string = dayjs(date1).utc(false).format(format);
    const max: string = dayjs(date2).utc(false).format(format);
    ...
    items.push(min);
    do {
      items.push(dayjs(items[i]).add(calcDuration).utc(false).format(format));
      if (msDuration === 31536000000 && items[i].slice(4, 10) !== items[i + 1].slice(4, 10)) {
        items[i + 1] = dayjs(items[i])
  ...
  getDateFragmentsOrder(dateFormat?: string): TypeDateFragments {
    ...
    */
    ...
    if (dateFormat) {
      ...
      dateFormat = dateFormat.replace(/z/gi, '+00:00');
      for (let i = 0; i < dateFormat.length; i++) {
        if (['/', '-', ' ', 'T', '+'].includes(dateFormat[i].toUpperCase())) dateFragmentsOrder[2].push(dateFormat[i]);
      }
      if (
      ...
      const dateTimeFragments = dateFormat.replaceAll(' ', 'T').toUpperCase().split('T');
      const dateFragments = dateTimeFragments[0].replaceAll('/', '-').toUpperCase().split('-');
      ...
      if (dateFragmentsOrder[0][0] === -1 || (dateFragmentsOrder[0][1] === -1 && dateFragmentsOrder[0][2] !== -1))
        throw new Error(`The string "${dateFormat}" is an invalid date format.`);

      if (dateTimeFragments[1]) {
        dateFragmentsOrder[0][3] = 3;
        dateFragmentsOrder[1][3] = 3;
        for (let i = 0; i < dateTimeFragments[1].length; i++) {
          if (['-', '+'].includes(dateTimeFragments[1][i])) {
            dateFragmentsOrder[2][4] = dateTimeFragments[1].slice(i + 1);
            break;
          }
        }
      }
  ...
  applyInputDateFormat(date: string, dateFragmentsOrder: TypeDateFragments, reverseTimeZone = false): string {
    ...
    const dateTimeFragments = date.replaceAll(' ', 'T').toUpperCase().split('T');
    if (!dateTimeFragments[1]) dateTimeFragments[1] = '00:00:00';
    const dateFragments = dateTimeFragments[0]
      .replaceAll('/', '-')
      .toUpperCase()
      .split('-')
      .map((fragment) => {
    ...
    if (returnValue.length === 25 && !returnValue.slice(-6).match(/[+-]\d\d:\d\d/))
      returnValue = `${returnValue}${dateFragmentsOrder[2][3]}${dateFragmentsOrder[2][4]}`;
    else if (returnValue.length === 19) returnValue = `${returnValue}+00:00`;
    if (reverseTimeZone)
      returnValue = `${returnValue.slice(0, 19)}${returnValue.slice(19, 20) === '+' ? '-' : '+'}${returnValue.slice(20)}`;
    return this.convertToUTC(returnValue);
  }
  ...
  applyOutputDateFormat(date: string, dateFragmentsOrder: TypeDateFragments): string {
    if (dateFragmentsOrder?.length) {
      const dateTimeFragments = date.toUpperCase().split('T');
      const dateFragments = dateTimeFragments[0].replaceAll('/', '-').toUpperCase().split('-');
      const returnValue = `${dateFragments[dateFragmentsOrder[1][0]]}${
        ...
        dateFragmentsOrder[1][3] + 1 ? `${dateFragmentsOrder[2][2]}${dateTimeFragments[1].slice(0, 8)}` : ''
      }`;
      return returnValue;
    }
    return date;

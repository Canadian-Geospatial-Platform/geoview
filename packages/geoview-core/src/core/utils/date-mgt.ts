...
export class DateMgt {
  ...
  getDateFragmentsOrder(dateFormat?: string): TypeDateFragments {
    ...
    */
    ...
    if (dateFormat) {
      ...
      const dateTimeFragments = dateFormat.replaceAll(' ', 'T').split('T');
      const dateFragments = dateTimeFragments[0].replaceAll('/', '-').split('-');
  ...
  applyInputDateFormat(date: string, dateFragmentsOrder: TypeDateFragments, reverseTimeZone = false): string {
    ...
    const dateTimeFragments = date.toUpperCase().replace('Z', '+00:00').replaceAll(' ', 'T').split('T');

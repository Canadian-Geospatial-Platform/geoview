...
export function isJsonString(str: string): boolean {
  try {
    ...
    logger.logError('- Invalid JSON string. String passed to the JSON parser:');
    logger.logError(str);
    logger.logError('- JSON Parser error:', (e as { message: string }).message);
    logger.logError('- See text above.');
    return false;
  }
  return true;
}

...
/**
 * Defines a generic delegate to be able to track an object inside the logTracker
 */
interface LogTrackerObjectCallback<T> {
  (objects?: T[]): {
    changed: boolean;
    objects: T[];
  };
...
export class ConsoleLogger {
   ...
   * Only shows if LOG_ACTIVE is true.
   * @param {string} message useEffectFunction the useEffect function identifier
   * @param {unknown[]} messages the messages to log
   */
   ...
   * Only shows if LOG_ACTIVE is true.
   * @param {string} message storeSubscription the store subscription event that was raised
   * @param {unknown[]} messages the messages to log
   */
   ...
   * @param {LogTrackerObjectCallback<T>} callbackCheck the callback to execute every interval to check if object has changed and log if so
   * @param {number} interval? the interval to call the callback for
   */
  logTrackerStart = <T>(trackerKey: string, callbackCheck: LogTrackerObjectCallback<T>, interval?: number): void => {
    // Validate log active and existing tracker clearing
    if (!LOG_ACTIVE) return;
    if (this.trackers[trackerKey]) this.logTrackerStop(trackerKey);

    // Calback right away on start
    let check = callbackCheck();

    // Redirect right away for the first tracker to happen immediately
    this.logLevel(LOG_DEBUG, 'TRAKR', 'goldenrod', ...check.objects, `(${trackerKey})`); // Not a typo, 5 characters for alignment

    // Start the interval to check every few ms
    this.trackers[trackerKey] = setInterval(() => {
      // Calback right away on start
      check = callbackCheck(check.objects);
...
// logger.logTrackerStart('test object tracker');

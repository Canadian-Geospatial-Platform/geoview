...
export class GeoUtilities {
  ...
  */
   ...
   */
  static ensureServiceRequestUrlGetFeature(url: string, layerId: string, version: string, outputFormat: string | undefined): string {
    // Redirect
    const outputFormatClause = outputFormat ? `&outputFormat=${encodeURIComponent(outputFormat)}` : '';
    return `${this.ensureServiceRequestUrl(url, 'WFS', 'GetFeature', version)}&typeName=${encodeURIComponent(layerId)}${outputFormatClause}`;
  }

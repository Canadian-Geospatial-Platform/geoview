...
function searchUniqueValueEntry(
  ...
  for (let i = 0; i < uniqueValueStyleInfo.length; i++) {
    ...
    let foundCount = 0;
    for (let j = 0; j < fields.length && foundCount < fields.length; j++) {
      ...
      // Check if the actual value is in fact a domain lookup
      const fieldMetadata = domainsLookup?.find((domain) => domain.name === fieldName);

      // If found
      if (fieldMetadata && fieldMetadata.domain && fieldMetadata.domain.codedValues) {
        // Try to find it
        const codedValue = (fieldMetadata.domain.codedValues as TypeJsonArray).find((dom) => dom.name === actualValue);

        // If found
        if (codedValue) actualValue = codedValue.code;
    ...
    // If found all the necessary fields key
    if (foundCount === fields.length) {

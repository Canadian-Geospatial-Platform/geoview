...
export abstract class Projection {
  ...
  /**
   * Determines whether the provided feature records contain actual attribute fields
   * (normal key/value properties) or if they only contain a single HTML/plain-text
   * payload, which is typical for WMS GetFeatureInfo responses that cannot return
   * structured attributes.
   * This helper is used to differentiate between:
   *   • **Standard feature info** — records with multiple named fields (e.g., { field1, field2, ... })
   *   • **Fallback WMS feature info** — records where the service returned only:
   *        { html: "<table>..." }  or  { plain_text: "..." }
   * Behavior:
   *   - For **WMS layers**, if the first record contains exactly one property named
   *     `"html"` or `"plain_text"`, the function returns `false`, indicating that
   *     no actual attribute fields are present.
   *   - For all other cases, it returns `true`.
   * @param {AbstractBaseLayerEntryConfig} layerConfig
   *        The layer configuration object used to determine whether the layer is a WMS layer.
   * @param {TypeFeatureInfoEntry[]} arrayOfRecords
   *        The array of feature info records returned by a GetFeatureInfo or data query.
   * @returns {boolean}
   *          Returns `true` if the records contain real attribute fields.
   *          Returns `false` if the records only contain a single HTML or plain-text payload
   *          typical of WMS fallback GetFeatureInfo responses.
   */

...
import type { LayerItemVisibilityToggledDelegate, LayerItemVisibilityToggledEvent } from '@/core/controllers/layer-controller';
import type {
  DomainLayerDelegate,
...
export class LayerApi {
  ...
  initEventsOnDomain(layerDomain: LayerDomain): void {
    ...
    this.#controllers.layerController.onLayerItemVisibilityToggled((sender, event) => {
      // Re-emit so the layer api can alert external devs
  ...
  setItemVisibility(layerPath: string, item: TypeLegendItem, visibility: boolean, refresh: boolean, waitForRender: boolean): Promise<void> {
    // Redirect to controller
    return this.#controllers.layerController.setItemVisibility(layerPath, item, visibility, refresh, waitForRender);
  }
  ...
  async loadListOfGeoviewLayer(mapConfigLayerEntries: MapConfigLayerEntry[]): Promise<void> {
    ...
    this.#controllers.mapController.setMapOrderedLayerInfoDirectly(orderedLayerInfos);
  }
  ...
  async addGeoviewLayerByGeoCoreUUID(uuid: string, layerEntryConfig?: string): Promise<GeoViewLayerAddedResult | undefined> {
    ...
    try {
      ...
      this.#controllers.mapController.addOrderedLayerInfo(layerInfo);
      ...
      if (geoviewLayerConfig.useAsBasemap === true) {
        ...
        this.#controllers.mapController.removeOrderedLayerInfo(geoviewLayerConfig.geoviewLayerId, true);
      }
      ...
      if (orderedInfo) this.#controllers.mapController.removeOrderedLayerInfo(uuid, false);
  ...
  reloadGeocoreLayers(): void {
    ...
    Promise.allSettled(
      ...
      .then(() => {
        ...
        this.#controllers.mapController.setMapOrderedLayerInfoDirectly(originalMapOrderedLayerInfo);
  ...
  removeLayerUsingPath(layerPath: string): void {
    ...
    if (layerEntryConfig) {
      ...
      this.#controllers.layerController.checkIfAllLayersLoadedAndUpdateStore();
  ...
  #addToMap(geoviewLayer: AbstractGeoViewLayer, geoviewLayerConfig: TypeGeoviewLayerConfig): void {
    ...
    if (!geoviewLayer.allLayerStatusAreGreaterThanOrEqualTo('error')) {
      ...
      this.#controllers.mapController.setLayerZIndices();
    }
  }
  ...
  #unregisterLayerConfig(layerConfig: ConfigBaseClass, unregisterOrderedLayerInfo: boolean = true): void {
    ...
    if (unregisterOrderedLayerInfo) {
      ...
      this.#controllers.mapController.removeOrderedLayerInfo(layerConfig.layerPath);
    }
      ...
      const validGeoviewLayerConfigs = mapConfigLayerEntries.filter((geoviewLayerConfigToCreate, configToCreateIndex) => {
        for (let configToTestIndex = 0; configToTestIndex < mapConfigLayerEntries.length; configToTestIndex++) {
          if (
            ...
            if (orderedInfo) {
              this.#controllers.mapController.removeOrderedLayerInfo(geoviewLayerConfigToCreate.geoviewLayerId, false);
            }
  ...
  static convertMapConfigsToGeoviewLayerConfig(
    ...
    return mapConfigLayerEntries.map((entry) => {
      // Redirect
      return LayerApi.convertMapConfigToGeoviewLayerConfig(mapId, currentLayerIds, language, entry, errorCallback);
    });
  }

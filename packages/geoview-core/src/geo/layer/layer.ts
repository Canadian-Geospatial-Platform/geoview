...
export class LayerApi {
  ...
  // Keep all callback delegates references
  #onLayersLoadedHandlers: LayersLoadedDelegate[] = [];
      ...
      const promiseLayer = new Promise<void>((resolve, reject) => {
        ...
        layerBeingAdded!
          ...
          .then(() => {
            ...
            layerBeingAdded!.onLayerLoaded((payload, sender) => {
              // Log
              logger.logDebug(`${sender.layerPath} loaded on map ${this.getMapId()}`);
              this.#emitLayersLoaded({ layerPath: sender.layerPath });
            });
   ...
   * @param {LayersLoadedEvent} event - The event to emit
   * @private
   */
  #emitLayersLoaded(event: LayersLoadedEvent): void {
    // Emit the event for all handlers
    EventHelper.emitEvent(this, this.#onLayersLoadedHandlers, event);
  }

  /**
   * Registers a layer loaded event handler.
   * @param {LayersLoadedDelegate} callback - The callback to be executed whenever the event is emitted
   */
  onLayersLoaded(callback: LayersLoadedDelegate): void {
    // Register the event handler
    EventHelper.onEvent(this.#onLayersLoadedHandlers, callback);
  }

  /**
   * Unregisters a layer loaded event handler.
   * @param {LayersLoadedDelegate} callback - The callback to stop being called whenever the event is emitted
   */
  offLayersLoaded(callback: LayersLoadedDelegate): void {
    // Unregister the event handler
    EventHelper.offEvent(this.#onLayersLoadedHandlers, callback);
  }
 ...
 * Define a delegate for the event handler function signature
 */
type LayersLoadedDelegate = EventDelegateBase<LayerApi, LayersLoadedEvent, void>;

/**
 * Define an event for the delegate
 */
export type LayersLoadedEvent = {
  // The loaded layer
  layerPath: string;
};

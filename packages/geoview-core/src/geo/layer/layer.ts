...
export class LayerApi {
  ...
  #addGeoviewLayerStep2(geoviewLayerConfig: TypeGeoviewLayerConfig): GeoViewLayerAddedResult | undefined {
    ...
    if (layerBeingAdded) {
      ...
      // Register layer creation event for the new GVLayers process
      layerBeingAdded.onLayerCreation((geoviewLayer: AbstractGeoViewLayer, event: LayerCreationEvent) => {
        // Log
        logger.logDebug(`Layer entry config created for ${event.config.layerPath} on map ${this.mapId}`, event.config);
  ...
  registerLayerConfigInit(layerConfig: TypeLayerEntryConfig): void {
    ...
    // Get the layer to register immediately, so the others happening in processListOfLayerEntryConfig get processed after those herein?
   ...
   */
  setLayerConfigObsolete(layerConfig: TypeLayerEntryConfig): void {
  ...
  async #registerForTimeSlider(layerConfig: TypeLayerEntryConfig): Promise<void> {
    ...
    try {
      ...
      geoviewLayer = this.getGeoviewLayerHybrid(layerConfig.layerPath);
    } catch (error) {

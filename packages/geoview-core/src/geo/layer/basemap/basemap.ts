...
export class Basemap {
  ...
  async createCoreBasemap(
    ...
    if (coreBasemapOptions) {
      ...
      if (coreBasemapOptions.basemapId === 'satellite' && this.basemapsList[projectionCode].satellite) {
        const satelliteLayer = await this.#createBasemapLayer(
          'satellite',
          this.basemapsList[projectionCode].satellite,
          coreBasemapOptions.shaded ? 0.75 : defaultOpacity,
          true
        );
        if (satelliteLayer) {
          basemapLayers.push(satelliteLayer);
          basemaplayerTypes.push('satellite');

          // Set default origin,extent,resolutions from layer
          defaultOrigin = satelliteLayer.origin;
          defaultExtent = satelliteLayer.extent;
          defaultResolutions = satelliteLayer.resolutions;
          minZoom = satelliteLayer.minScale;
          maxZoom = satelliteLayer.maxScale;
        }
      }

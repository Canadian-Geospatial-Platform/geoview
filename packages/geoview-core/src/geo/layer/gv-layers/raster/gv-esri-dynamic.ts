...
export class GVEsriDynamic extends AbstractGVRaster {
  ...
  protected override async getFeatureInfoAtLongLat(
    ...
    if (queryGeometry)
      // TODO: REFACTOR - Here, we're launching another async task, but the original promise will resolve first, by design.
      ...
      // TO.DOCONT: This would mean change the 'getFeatureInfoAtLongLat' function signature with regards to its return type (and affect ALL other sibling classes)
  ...
  override async getExtentFromFeatures(objectIds: string[], outfield?: string): Promise<Extent | undefined> {
    ...
    if (baseUrl) {
      ...
      try {
        const responseJson = await Fetch.fetchJson(queryUrl);

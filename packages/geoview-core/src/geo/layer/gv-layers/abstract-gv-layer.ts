...
export abstract class AbstractGVLayer {
   ...
   * @param {AbstractBaseLayerEntryConfig} layerEntryConfig - The layer configuration.
   */
  constructor(mapId: string, olLayer: BaseLayer, layerEntryConfig: AbstractBaseLayerEntryConfig) {
    this.#mapId = mapId;
    this.#olLayer = olLayer;
    this.#layerEntryConfig = layerEntryConfig;
    this.#layerStatus = 'loading';

    // Depending on the instance
    let listenerName;
    if (olLayer instanceof ImageLayer) {
      listenerName = 'image';
    } else if (olLayer instanceof VectorTileLayer) {
      listenerName = 'tile';
    } else if (olLayer instanceof BaseVectorLayer) {
      listenerName = 'features';
    } else {
      // Unsupported
      throw new Error(`Unsupported OpenLayer type: ${olLayer.constructor.name}`);
    }

    // Keep the date formatting information
    this.#serverDateFragmentsOrder = layerEntryConfig.geoviewLayerConfig.serviceDateFormat
      ? DateMgt.getDateFragmentsOrder(layerEntryConfig.geoviewLayerConfig.serviceDateFormat)
      : undefined;
    this.#externalFragmentsOrder = DateMgt.getDateFragmentsOrder(layerEntryConfig.geoviewLayerConfig.externalDateFormat);

    // TODO: Check if the open layer source is already loaded status, before wiring the handlers here for nothing
    // Activation of the load end listeners
    olLayer.get('source').once(`${listenerName}loaderror`, this.onLoaded.bind(this));
    olLayer.get('source').once(`${listenerName}loadend`, this.onError.bind(this));
  }
   ...
   */
  getLayerPath(): string {
    return this.#layerEntryConfig.layerPath;
  }
  ...
  getGeoviewLayerId(): string {
    return this.#layerEntryConfig.geoviewLayerConfig.geoviewLayerId;
  }
  ...
  getGeoviewLayerName(): TypeLocalizedString | undefined {
    return this.#layerEntryConfig.geoviewLayerConfig.geoviewLayerName;
  }
  ...
  getLayerConfigStatus(): TypeLayerStatus {
    return this.#layerEntryConfig.layerStatus;
  }
  ...
  protected onLoaded(): void {
    ...
    this.setVisible(this.#layerEntryConfig.initialSettings?.states?.visible !== false);
  }

...
export interface TypeWmtsConfig extends TypeGeoviewLayerConfig {
...
export class WMTS extends AbstractGeoViewRaster {
  ...
  constructor(layerConfig: TypeWmtsConfig) {
    super(layerConfig);
  }
  ...
  protected override onFetchServiceMetadata<T = TypeMetadataWMTS | undefined>(abortSignal?: AbortSignal): Promise<T> {
    ...
    const isXml = WMTS.#isXmlMetadata(this.getMetadataAccessPath());

    if (isXml) {
      // Fetch the XML
      return this.#fetchXmlServiceMetadata(this.getMetadataAccessPath(), abortSignal) as Promise<T>;
    }
  ...
  protected override async onProcessLayerMetadata(
    ...
    if (!metadataTileMatrixFound || !metadataLayerFound) {
      ...
      throw new LayerServiceMetadataUnableToFetchError(
        this.getGeoviewLayerId(),
        this.getLayerEntryNameOrGeoviewLayerName(),
        formatError(`No matching TileMatrixSet or Layer found in metadata for layer ID ${layerConfig.layerId}`)
      );
    }
    ...
    if (!layerConfig.tileMatrixSet) {
      ...
      if (!layerMetadataTileMatrixSetLink) {
        ...
        throw new LayerServiceMetadataUnableToFetchError(
          this.getGeoviewLayerId(),
          this.getLayerEntryNameOrGeoviewLayerName(),
          formatError(`No TileMatrixSetLink matching ${layerConfig.tileMatrixSet} found in metadata for Layer ${layerConfig.layerId}`)
        );
      }
    }
    ...
    if (!layerConfig.hasDataAccessPath()) {
      ...
      if (tileLink) {
        ...
        throw new LayerServiceMetadataUnableToFetchError(
          this.getGeoviewLayerId(),
          this.getLayerEntryNameOrGeoviewLayerName(),
          formatError(`No GetTile operation with KVP encoding found in metadata for layer ID ${layerConfig.layerId}`)
        );
      }
    }
    ...
    await Projection.addProjectionIfMissing(metadataProjectionCode);
   ...
   * the WMTS GetCapabilities convention.
   *
   * @param path - The metadata URL to check.
   * @returns `true` if the URL ends with `.xml`, otherwise `false`.
   */
  ...
  static initGeoviewLayerConfig(
    ...
    const myLayer = new WMTS({ geoviewLayerId, geoviewLayerName, metadataAccessPath, isTimeAware } as TypeWmtsConfig);
  ...
  static createGeoviewLayerConfig(
    ...
    const geoviewLayerConfig: TypeWmtsConfig = {
      geoviewLayerId,
      geoviewLayerName,
      metadataAccessPath,
   ...
   * @throws {LayerServiceMetadataUnableToFetchError} When the tileMatrixSet and layer information are missing, likely because onProcessMetadata wasn't called.
   */
  static createWMTSSource(layerConfig: OgcWmtsLayerEntryConfig): WMTSSource {
    ...
    if (!layer || !tileMatrixSet) {
      throw new LayerServiceMetadataUnableToFetchError(
        ...
        formatError(
          'Missing metadata information. Ensure onProcessLayerMetadata has been called successfully before creating the WMTS source.'
        )
      );
    }

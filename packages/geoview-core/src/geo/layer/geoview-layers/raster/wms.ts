      ...
      if (geoviewEntryIsWMS(layerEntryConfig)) {
        ...
        if (layerCapabilities) {
          ...
          if (layerCapabilities.Dimension) {
            let i: number;
            for (i = 0; i < layerCapabilities.Dimension.length; i++) if (layerCapabilities.Dimension[i].name === 'time') break;
            if (i < layerCapabilities.Dimension.length)
              this.processTemporalDimension(layerCapabilities.Dimension[i] as TypeJsonObject, layerEntryConfig);
          }
        }
      }
      resolve();

...
export class WMS extends AbstractGeoViewRaster {
  ...
  #createGroupLayer(layer: TypeJsonObject, layerConfig: GroupLayerEntryConfig): void {
    ...
    // TODO: Bug - There was a long term issue in this loop which was causing it to fail and
    // TO.DOCONT: not process the other sub layers. Fixing the cloning issue ('Private element not on object')
    // TO.DOCONT: produces a behavior we actually don't want at the moment and we'll address only later.
    // TO.DOCONT: Therefore, we're making it crash on purpose for now to keep the behavior the same as before..

...
export class GeoTIFF extends AbstractGeoViewRaster {
  ...
  static createGeoTIFFSource(layerConfig: GeoTIFFLayerEntryConfig): GeoTIFFSource {
    ...
    const geoTiffSource = new GeoTIFFSource(sourceOptions);

    // Attach error listener IMMEDIATELY before any async operations can complete
    const handleError = (): void => {
      const state = geoTiffSource.getState();
      if (state === 'error') {
        // eslint-disable-next-line no-underscore-dangle, @typescript-eslint/no-explicit-any
        const error = (geoTiffSource as any).error_;
    ...
    // TODO: Handle cleanup of listener when source is deleted
    geoTiffSource.on('change', handleError);

    // Also check current state in case it already errored
    handleError();

...
import { CancelledError, ResponseEmptyError, PromiseRejectErrorWrapper } from '@/core/exceptions/core-exceptions';
...
export abstract class AbstractGeoViewLayer {
  ...
  /**
   * Calls this.addLayerLoadError for each layer entry config found in the list.
   * @param {TypeLayerEntryConfig[]} listOfLayerEntryConfig - The list of layer entry configs
   * @param {Error} error - The error
   */
  #addLayerLoadErrorAll(listOfLayerEntryConfig: TypeLayerEntryConfig[], error: Error): void {
    listOfLayerEntryConfig.forEach((layerConfig: TypeLayerEntryConfig) => {
      // If the layer entry is a group
      if (layerEntryIsGroupLayer(layerConfig)) {
        // Recursively set the status to the children
        this.#addLayerLoadErrorAll(layerConfig.listOfLayerEntryConfig, error);
        // And set the group layer itself in error
        this.addLayerLoadError(error, layerConfig);
      } else {

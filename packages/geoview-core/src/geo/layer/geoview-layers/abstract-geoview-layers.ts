  ...
  queryLegend(layerPath: string): Promise<TypeLegend | null> {
    ...
    promiseLegend
      .then((legend) => {
        ...
        if (legend) {
          // Check for possible number of icons to set icon cache
            ...
            Object.keys(legend.styleConfig).forEach((geometry) => {
              if (
                ...
                if ((legend.styleConfig[geometry as TypeStyleGeometry] as TypeUniqueValueStyleConfig)!.uniqueValueStyleInfo?.length)
                  styleCount += (legend.styleConfig[geometry as TypeStyleGeometry] as TypeUniqueValueStyleConfig)!.uniqueValueStyleInfo
                    .length;
                if ((legend.styleConfig[geometry as TypeStyleGeometry] as TypeClassBreakStyleConfig)!.classBreakStyleInfo?.length)
                  styleCount += (legend.styleConfig[geometry as TypeStyleGeometry] as TypeClassBreakStyleConfig)!.classBreakStyleInfo
                    .length;
              }
            });

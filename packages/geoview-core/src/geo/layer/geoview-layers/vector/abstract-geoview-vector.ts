...
export abstract class AbstractGeoViewVector extends AbstractGeoViewLayer {
  ...
  getAllFeatureInfo(layerId?: string): TypeFeatureInfoResult {
    const layer = layerId ? this.getBaseLayer(layerId) : this.activeLayer;
  ...
  protected addFieldEntryToSourceFeatureInfo = (
    ...
    fieldName: 'outfields' | 'aliasFields', // Propery name
    fieldValue: string, // value to append to the property
    prefixEntryWithComa: number // flag (0 = false) indicating that we must prefix the entry With a ', '
  ) => {

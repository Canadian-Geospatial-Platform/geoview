...
export class GeoJSON extends AbstractGeoViewVector {
  ...
  private processLayerMetadata(layerEntryConfig: TypeVectorLayerEntryConfig): Promise<void> {
    const promiseOfExecution = new Promise<void>((resolve) => {
      ...
      for (var i = 0; i < metadataLayerList.length; i++) if (metadataLayerList[i].layerId === layerEntryConfig.layerId) break;
      const metadataLayerSource = metadataLayerList[i].source;
      if (metadataLayerSource) {
        if (!layerEntryConfig.source) layerEntryConfig.source = metadataLayerSource;
        else {
          if (!layerEntryConfig.source.cluster) layerEntryConfig.source.cluster = metadataLayerSource.cluster;
          if (!layerEntryConfig.source.featureInfo) layerEntryConfig.source.featureInfo = metadataLayerSource.featureInfo;
          else {
            if (!layerEntryConfig.source.featureInfo.aliasFields)
              layerEntryConfig.source.featureInfo.aliasFields = metadataLayerSource.featureInfo?.aliasFields;
            if (!layerEntryConfig.source.featureInfo.customParser)
              layerEntryConfig.source.featureInfo.customParser = metadataLayerSource.featureInfo?.customParser;
            if (!layerEntryConfig.source.featureInfo.nameField)
              layerEntryConfig.source.featureInfo.nameField = metadataLayerSource.featureInfo?.nameField;
            if (!layerEntryConfig.source.featureInfo.outfields)
              layerEntryConfig.source.featureInfo.outfields = metadataLayerSource.featureInfo?.outfields;
            if (!layerEntryConfig.source.featureInfo.queryable)
              layerEntryConfig.source.featureInfo.queryable = !!metadataLayerSource.featureInfo?.queryable;
          }
        }
      }
      const metadataLayerInitialSettings = metadataLayerList[i].initialSettings;
      if (metadataLayerInitialSettings) {
        if (!layerEntryConfig.initialSettings) layerEntryConfig.initialSettings = metadataLayerInitialSettings;
        else {
          if (!layerEntryConfig.initialSettings.className)
            layerEntryConfig.initialSettings.className = metadataLayerInitialSettings.className;
          if (!layerEntryConfig.initialSettings.extent) layerEntryConfig.initialSettings.extent = metadataLayerInitialSettings.extent;
          if (!layerEntryConfig.initialSettings.maxZoom) layerEntryConfig.initialSettings.maxZoom = metadataLayerInitialSettings.maxZoom;
          if (!layerEntryConfig.initialSettings.minZoom) layerEntryConfig.initialSettings.minZoom = metadataLayerInitialSettings.minZoom;
          if (!layerEntryConfig.initialSettings.opacity) layerEntryConfig.initialSettings.opacity = metadataLayerInitialSettings.opacity;
          if (!layerEntryConfig.initialSettings.visible) layerEntryConfig.initialSettings.visible = metadataLayerInitialSettings.visible;
        }
      }
      ...
      const metadataLayerStyle = metadataLayerList[i].style;
      if (metadataLayerStyle && !layerEntryConfig.style) layerEntryConfig.style = metadataLayerStyle;
      resolve();
    });

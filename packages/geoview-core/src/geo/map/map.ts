...
export class MapViewer {
   ...
   * @param {TypeListOfGeoviewLayerConfig} geoviewLayerConfi optional new set of layers to apply (will override original set of layers)
   */
  changeLanguage = (displayLanguage: TypeDisplayLanguage, listOfGeoviewLayerConfig?: TypeListOfGeoviewLayerConfig): void => {
    ...
    // if a list of geoview layers parameter is present, replace the one fromt the config
  ...
  loadMapConfig = (mapConfig: string) => {
    ...
    // if we do not so this, it creates a new map id in the array of maps instead of replacing.

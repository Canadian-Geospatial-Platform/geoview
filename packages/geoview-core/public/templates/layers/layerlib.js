...
const addBoundsPolygon = (mapId, bbox) => {
  const newBbox = cgpv.api.utilities.projection.transformAndDensifyExtent(
    bbox,
...
const createTableOfFilter = (mapId) => {
  ...
  let geoviewLayerIds = cgpv.api.maps?.[mapId]?.layer?.getGeoviewLayerIds();
  if (!geoviewLayerIds || geoviewLayerIds.length === 0) return;
  geoviewLayerIds.forEach((geoviewLayerId) => {
    const geoviewLayer = cgpv.api.maps[mapId].layer.getGeoviewLayer(geoviewLayerId);

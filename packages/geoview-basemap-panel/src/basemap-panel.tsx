...
export function BasemapPanel(props: BaseMapPanelProps): JSX.Element {
  ...
  const useStyles = ui.makeStyles(() => ({
    ...
    active: {
      boxShadow: '0 16px 32px 0 rgba(0, 0, 0, 0.8) !important',
    },
    card: {
  ...
  const createBasemapArray = async (projection: number) => {
    const basemapsArray: TypeJsonObject = (config.supportedProjections as Array<TypeJsonObject>).find(
      (obj: TypeJsonObject) => obj.projectionCode === projection
    );
    let isInit = false;
  ...
  const setSelectedProjection = (event: TypeSelectChangeEvent<unknown>) => {
    ...
    const newView: TypeMapView = {
      ...
      projection: newProjection as Projection,
    };

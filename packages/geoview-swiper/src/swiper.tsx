...
export function Swiper(props: SwiperProps): JSX.Element {
  ...
  const [olLayers, setOlLayers] = useState([]);
   ...
   * @param {RenderEvent} evt The pre compose, pre render event
   * @private
   */
  function prerender(evt: RenderEvent) {
   ...
   * @param {RenderEvent} evt the post compose, post render event
   * @private
   */
  function postcompose(evt: RenderEvent) {
    const ctx: CanvasRenderingContext2D | WebGLRenderingContext = evt.context!;
  ...
  const onStop = debounce((evt: MouseEvent) => {
    ...
    olLayers.forEach((layer: BaseLayer) => {
      if (typeof layer.getImageRatio === 'function') layer?.changed();
    });
  ...
  useEffect(() => {
    ...
    layersIds.forEach((layer: string) => {
      ...
      olLayer?.on(['precompose', 'prerender'], prerender);
      olLayer?.on(['postcompose', 'postrender'], postcompose);

      // force VectorImage to refresh
      if (typeof olLayer?.getImageRatio === 'function') olLayer?.changed();
    });

    return () => {
      layersIds.forEach((layer: string) => {
        const olLayer = geoviewLayers[`${layer}`].gvLayers;
        olLayer?.un(['precompose', 'prerender'], prerender);
        olLayer?.un(['postcompose', 'postrender'], postcompose);

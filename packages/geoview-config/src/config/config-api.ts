...
export class ConfigApi {
  ...
  static validateBasemap(projection?: TypeValidMapProjectionCodes, basemapOptions?: TypeJsonObject): TypeBasemapOptions {
    if (projection && basemapOptions) {
      const basemapId = BASEMAP_ID[projection].includes(basemapOptions.basemapId as TypeBasemapId)
        ? basemapOptions.basemapId
        : DEFAULT_MAP_FEATURES_CONFIG.gvMap.basemapOptions.basemapId;
      const shaded = BASEMAP_SHADED[projection].includes(basemapOptions.shaded as boolean)
        ? basemapOptions.shaded
        : DEFAULT_MAP_FEATURES_CONFIG.gvMap.basemapOptions.shaded;
      const labeled = BASEMAP_LABEL[projection].includes(basemapOptions.labeled as boolean)
  ...
  private static validateCenter(projection?: TypeValidMapProjectionCodes, center?: TypeJsonArray): [number, number] {
    ...
    if (center) {
      if (projection) {
        ...
        const x =
          !Number.isNaN(xVal) && xVal > MAP_CENTER[projection].long[0] && xVal < MAP_CENTER[projection].long[1]
            ? xVal
            : gvMap.viewSettings.center[0];
        const y =
          !Number.isNaN(yVal) && yVal > MAP_CENTER[projection].lat[0] && yVal < MAP_CENTER[projection].lat[1]
            ? yVal
  ...
  private static validateExtent(
    ...
    if (projection && extent && extent.length === 4) {
      ...
      const minX = !Number.isNaN(extentMinX) && (extentMinX as number) < center[0] ? extentMinX : MAP_CENTER[projection].long[0];
      const minY = !Number.isNaN(extentMinY) && (extentMinY as number) < center[1] ? extentMinY : MAP_CENTER[projection].lat[0];
      const maxX = !Number.isNaN(extentMaxX) && (extentMaxX as number) > center[0] ? extentMaxX : MAP_CENTER[projection].long[1];
      const maxY = !Number.isNaN(extentMaxY) && (extentMaxY as number) > center[1] ? extentMaxY : MAP_CENTER[projection].lat[1];

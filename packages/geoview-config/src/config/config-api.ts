...
import { VALID_BASEMAP_ID, CONST_LAYER_TYPES, CONST_GEOVIEW_SCHEMA_BY_TYPE, DEFAULT_MAP_FEATURES_CONFIG } from './types/config-constants';
import { Cast, TypeGeoviewLayerType, TypeJsonArray, TypeJsonObject, TypeJsonValue, toJsonObject } from './types/config-types';
import {
  TypeDisplayLanguage,
...
export class ConfigApi {
  ...
  // valid basemap ids
  static #basemapId: Record<TypeValidMapProjectionCodes, TypeBasemapId[]> = {
    3857: VALID_BASEMAP_ID,
    3978: VALID_BASEMAP_ID,
  };

  // valid shaded basemap values for each projection
  static #basemapShaded: Record<TypeValidMapProjectionCodes, boolean[]> = {
    3857: [true, false],
    3978: [true, false],
  };

  // valid labeled basemap values for each projection
  static #basemaplabeled: Record<TypeValidMapProjectionCodes, boolean[]> = {
    3857: [true, false],
    3978: [true, false],
  };

  // valid center levels from each projection
  static #center: Record<TypeValidMapProjectionCodes, Record<string, number[]>> = {
  ...
  static get defaultMapFeaturesConfig(): MapFeaturesConfig {
    return new MapFeaturesConfig(JSON.stringify(DEFAULT_MAP_FEATURES_CONFIG));
  }
  ...
  static validateBasemap(projection?: TypeValidMapProjectionCodes, basemapOptions?: TypeJsonObject): TypeBasemapOptions {
    if (projection && basemapOptions) {
      const basemapId = ConfigApi.#basemapId[projection].includes(basemapOptions.basemapId as TypeBasemapId)
        ? basemapOptions.basemapId
        : DEFAULT_MAP_FEATURES_CONFIG.gvMap.basemapOptions.basemapId;
      const shaded = ConfigApi.#basemapShaded[projection].includes(basemapOptions.shaded as boolean)
        ? basemapOptions.shaded
        : DEFAULT_MAP_FEATURES_CONFIG.gvMap.basemapOptions.shaded;
      const labeled = ConfigApi.#basemaplabeled[projection].includes(basemapOptions.labeled as boolean)
        ? basemapOptions.labeled
        : DEFAULT_MAP_FEATURES_CONFIG.gvMap.basemapOptions.labeled;

      return { basemapId, shaded, labeled } as TypeBasemapOptions;
    }
    return DEFAULT_MAP_FEATURES_CONFIG.gvMap.basemapOptions;
  }
  ...
  static validateVersion(version?: TypeJsonValue): TypeValidVersions {
    return version && VALID_VERSIONS.includes(version as TypeValidVersions)
      ...
      : DEFAULT_MAP_FEATURES_CONFIG.schemaVersionUsed!;
  }
  ...
  private static validateZoom(zoom?: number): number {
    return zoom && !Number.isNaN(zoom) && zoom >= 0 && zoom <= 18 ? zoom : DEFAULT_MAP_FEATURES_CONFIG.gvMap.viewSettings.zoom;
  }
  ...
  private static validateProjection(projection?: TypeValidMapProjectionCodes): TypeValidMapProjectionCodes {
    return projection && VALID_PROJECTION_CODES.includes(projection)
      ? projection
      : DEFAULT_MAP_FEATURES_CONFIG.gvMap.viewSettings.projection;
  }
  ...
  private static validateCenter(projection?: TypeValidMapProjectionCodes, center?: TypeJsonArray): [number, number] {
    const { gvMap } = DEFAULT_MAP_FEATURES_CONFIG;
    if (center) {
      if (projection) {
        ...
        const x =
          !Number.isNaN(xVal) && xVal > ConfigApi.#center[projection].long[0] && xVal < ConfigApi.#center[projection].long[1]
            ? xVal
            : gvMap.viewSettings.center[0];
        const y =
          !Number.isNaN(yVal) && yVal > ConfigApi.#center[projection].lat[0] && yVal < ConfigApi.#center[projection].lat[1]
            ? yVal
  ...
  private static validateExtent(
    ...
    if (projection && extent && extent.length === 4) {
      ...
      const minX = !Number.isNaN(extentMinX) && (extentMinX as number) < center[0] ? extentMinX : ConfigApi.#center[projection].long[0];
      const minY = !Number.isNaN(extentMinY) && (extentMinY as number) < center[1] ? extentMinY : ConfigApi.#center[projection].lat[0];
      const maxX = !Number.isNaN(extentMaxX) && (extentMaxX as number) > center[0] ? extentMaxX : ConfigApi.#center[projection].long[1];
      const maxY = !Number.isNaN(extentMaxY) && (extentMaxY as number) > center[1] ? extentMaxY : ConfigApi.#center[projection].lat[1];
   ...
   */
  private static printSchemaError(validate: AnyValidateFunction<unknown>, objectAffected: unknown): void {
   ...
   */
  private static IsValidTypeMapFeaturesInstance(mapFeaturesConfigToValidate: TypeJsonObject, validator: Ajv): boolean {
    ...
    const valid = validate({ ...(mapFeaturesConfigToValidate as object) });

    if (!valid) {
      ConfigApi.printSchemaError(validate, mapFeaturesConfigToValidate);
      return false;
    }
    return true;
  }
   ...
   * @returns {MapFeaturesConfig} A valid map features configuration.
   */
  private static IsValidTypeListOfLayerEntryConfig(
    geoviewLayerType: TypeJsonValue | TypeGeoviewLayerType,
    listOfLayerEntryConfig: TypeJsonArray | ConfigBaseClass[],
    validator: Ajv
  ): boolean {
    const layerSchemaPath = `https://cgpv/schema#/definitions/${CONST_GEOVIEW_SCHEMA_BY_TYPE[geoviewLayerType as TypeGeoviewLayerType]}`;
    ...
    for (let i = 0; i < listOfLayerEntryConfig.length; i++) {
      ...
      if (!valid) {
        ConfigApi.printSchemaError(validate, listOfLayerEntryConfig[i]);
        return false;
      }
    }
    ...
    for (let i = 0; i < listOfLayerEntryConfig.length; i++) {
      ...
      if (
        ...
        !ConfigApi.IsValidTypeListOfLayerEntryConfig(geoviewLayerType, layerEntryConfig.listOfLayerEntryConfig!, validator)
      )
        return false;
    }
    return true;
   ...
   * @returns {MapFeaturesConfig} A valid map features configuration.
   */
  static validateMapConfigAgainstSchema(mapFeaturesConfigToValidate: TypeJsonObject): TypeJsonObject {
    ...
    let isValid = ConfigApi.IsValidTypeMapFeaturesInstance(mapFeaturesConfigToValidate, validator);
    for (let i = 0; i < (mapFeaturesConfigToValidate?.gvMap?.listOfGeoviewLayerConfig as TypeJsonArray).length && isValid; i++) {
      ...
      /* if (!mapConfigLayerEntryIsGeoCore(mapFeaturesConfigToValidate.gvMap.listOfGeoviewLayerConfig[i])) */ {
        const gvLayerConfig = mapFeaturesConfigToValidate?.gvMap.listOfGeoviewLayerConfig[i];
        isValid = ConfigApi.IsValidTypeListOfLayerEntryConfig(
          gvLayerConfig.geoviewLayerType,
          gvLayerConfig.listOfLayerEntryConfig as TypeJsonArray,
          validator
        );
      }
    }

    if (!isValid) validMapFeaturesConfig = ConfigApi.adjustMapConfiguration(mapFeaturesConfigToValidate);

    ConfigApi.processLocalizedString(
      validMapFeaturesConfig.suportedLanguages as TypeJsonArray,
      validMapFeaturesConfig.gvMap.listOfGeoviewLayerConfig as TypeJsonArray
    );
    ConfigApi.doExtraValidation(validMapFeaturesConfig.gvMap.listOfGeoviewLayerConfig as TypeJsonArray);

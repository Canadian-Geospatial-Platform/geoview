...
export class CoreTester extends GVAbstractTester {
  ...
  testDatesEpochTimestamps(): Promise<Test<DatesSet[]>> {
    ...
    return this.test(
      ...
      (test, result) => {
        ...
        CoreTester.#assertDatesFromEpochDatesStringWithZ(test, newYearEpoch, CoreTester.JAN1_2000_0000_ISO, CoreTester.JAN1_1999_1900_ISO);

        test.addStep(`-> Verifying dates from date object... ${newYearDate.input}`);
        CoreTester.#assertDatesFromEpochDatesStringWithZ(test, newYearDate, CoreTester.JAN1_2000_0000_ISO, CoreTester.JAN1_1999_1900_ISO);

        test.addStep(`-> Verifying dates from date ISO with Z... ${datesISOZ.input}`);
        CoreTester.#assertDatesFromEpochDatesStringWithZ(test, datesISOZ, CoreTester.JAN1_2000_0000_ISO, CoreTester.JAN1_1999_1900_ISO);

        test.addStep(`-> Verifying dates from epoch timestamps with long output format... ${newYearEpochLongFormat.input}`);
        CoreTester.#assertDatesFromEpochDatesStringWithZ(
          test,
          newYearEpochLongFormat,
   ...
   * @static
   */
  static #assertDatesFromEpochDatesStringWithZ(test: Test, result: DatesSet, dateUTC: string, dateBefore: string): void {

...
export function GeoChart(props: GeoChartProps): JSX.Element {
  ...
  const handleChartLoad = useCallback<(e: PayloadChartLoad) => void>((e: PayloadChartLoad): void => {
    // Log
    logger.logUseCallback('GEOVIEW-GEOCHART - handleChartLoad', e);
  ...
  const handleQueryStarted = useCallback<(e: PayloadBaseClass) => void>((e: PayloadBaseClass): void => {
    // Log
    logger.logUseCallback('GEOVIEW-GEOCHART - handleQueryStarted', e);
  ...
  const fetchDatasources = useCallback<(layerDataArray: TypeArrayOfLayerData) => void>(
    (layerDataArray: TypeArrayOfLayerData): void => {
      // Log
      logger.logUseCallback('GEOVIEW-GEOCHART - fetchDatasources', mapId, layerDataArray);
  ...
  const handleError = useCallback<(error: string, exception: unknown | undefined) => void>(
    (error: string): void => {
      // Log
      logger.logUseCallback('GEOVIEW-GEOCHART - handleError', mapId, error);
  ...
  useEffect(() => {
    ...
    logger.logUseEffectMount(USE_EFFECT_FUNC, storeLayerDataArray);
    ...
    return () => {
      // Log
      logger.logUseEffectUnmount(USE_EFFECT_FUNC, storeLayerDataArray);
    };
  }, [fetchDatasources, storeLayerDataArray]);
  ...
  useEffect(() => {
    ...
    logger.logUseEffectMount(USE_EFFECT_FUNC, mapId);
    ...
    return () => {
      // Log
      logger.logUseEffectUnmount(USE_EFFECT_FUNC, mapId);

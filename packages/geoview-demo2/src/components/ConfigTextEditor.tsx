...
export function ConfigTextEditor() {
  ...
  return (
    ...
    <Box className="config-editor">
      ...
      <textarea id="configGeoview" name="configuration" value={editorText} onChange={onTextareaChange} rows={30} cols={150} ref={textEditorRef}>
      </textarea>
    </Box>
    <Box sx={{ display: 'flex', justifyContent: 'center', mt: 2, gap: 2 }}>
      <Button variant="contained" color="primary" onClick={validateText} disabled={!isEditorTouched}>Validate</Button>
      <Button variant="contained" color="primary" onClick={createMap} disabled={!isValidJson}>Create Map</Button>
  </Box>
  </Box>
  );
}
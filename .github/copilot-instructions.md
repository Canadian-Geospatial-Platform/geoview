...
**GeoPackage feature-info is built by a SEPARATE inference path (not `AbstractGeoViewVector`)** — GeoPackage layers use the `GeoPackage` config type, which `GeoPackageReader.createLayerConfigFromGeoPackage()` converts to a **WKB** layer. The reader pre-populates each entry's `source.featureInfo.outfields` via its **own** `GeoPackageReader.#processFeatureInfoConfig()` ([geopackage-reader.ts](../packages/geoview-core/src/api/config/reader/geopackage-reader.ts)), reading `Object.keys(geoPackageFeatures[0].properties)`. Because `source.featureInfo` is already set, `AbstractGeoViewVector.processFeatureInfoConfig()`'s inference branch (`if (!outfields?.length)`) is **skipped** for GeoPackage. Consequences:

- **A fix in `AbstractGeoViewVector.processFeatureInfoConfig()` does NOT affect GeoPackage.** GeoJSON-by-inference and CSV go through the abstract-vector path; GeoPackage does not. To filter/transform GeoPackage outfields you must edit `GeoPackageReader.#processFeatureInfoConfig()`.
- **GeoPackage preserves a blank-named column verbatim.** `WKB.onCreateVectorSourceLoadFeatures()` does `feature.setProperties(properties)` copying every key including `''`, and the reader's inference has no empty-key guard (only a falsy-**value** guard `if (!fields[key]) return`). GeoJSON/CSV never carry a `''` field through to a column, so **only GeoPackage/WKB reproduces empty-field-name bugs**.
- **A single-table gpkg keeps the config `layerId`; an explicitly nested `listOfLayerEntryConfig` uses the table name.** For deterministic layer paths (e.g. in tests) nest the table entry: `railwaysEmptyField/carto_fer_debarcadere`, not `railwaysEmptyField/railways-emptyfieldname.gpkg`.

**Empty/blank field name → Material React Table crash** — MRT throws `Columns require an id when using an accessorFn` when a column has an `accessorFn` and a falsy/empty `id`. An unnamed column (`''`) becomes `{ id: '', accessorFn }` and takes down the viewer via `show-hide-columns-button.tsx` → `getAllLeafColumns()`. Defense-in-depth guards (all use `key.trim()` / `alias.trim()`):

- **Source (inference layers only):** `AbstractGeoViewVector.processFeatureInfoConfig()` early-returns on `!header.trim()`.
- **Consumers (catches GeoPackage too):** `memoColumns` in [data-table.tsx](../packages/geoview-core/src/core/components/data-table/data-table.tsx) (`if (!key.trim()) return`) and [data-table-modal.tsx](../packages/geoview-core/src/core/components/data-table/data-table-modal.tsx) (`key.trim()` in the push condition). These are what actually prevent the GeoPackage crash, since the reader source is not filtered.
- **Details table:** [feature-info-table.tsx](../packages/geoview-core/src/core/components/details/feature-info-table.tsx) strips both `geoviewID` and blank-alias rows (a blank alias renders a label-less row).
...
### Hidden Layers in Component Panel Lists (data table, details, geochart, time slider)

The list-based component panels keep visibility-hidden layers in a separate **"Hidden layers"** section (with an inline eye toggle) instead of dropping them. Key patterns learned building this (issue #3635):

- **`useStoreLayerIsHiddenOnMapSet` is a COMPOSITE flag.** `utilGetLayerHiddenOnMap` returns `parentHidden || !inVisibleRange || !visible`. To distinguish "visibility-toggle-hidden" from "out-of-scale-range" (which must stay filtered out), pair it with **`useStoreLayerInVisibleRangeSet()`**. Panel list-builder recipe: keep entries where `inVisibleRangeSet[path] !== false` (drops out-of-range), then flag `isHidden: layerHiddenSet[path]`. A layer is "visibility-hidden" only when `layerHiddenSet[path] && inVisibleRangeSet[path] !== false`.
- **Re-enabling a hidden child needs its parents.** `setOrToggleLayerVisibility(child, true)` will NOT render a child masked by a hidden parent group (parent-child visibility independence). Use **`LayerController.setLayerVisibleIncludingParents(layerPath)`**, which crawls `gvLayer.getParents()` and turns each hidden ancestor visible before the layer itself. Enabling a group re-renders its other individually-visible children (inherent to group visibility) — the crawl only turns ancestors on, it never changes sibling state.
- **`LayerListEntry.relatedLayerPaths`** — a multi-layer entry (custom time slider = main + `additionalLayerpaths`) re-enables ALL backing layers via one eye click. Its `isHidden` = `relatedLayerPaths.every((p) => layerHiddenSet[p] !== false)`, so untracked/legend-less layers count as hidden and the entry collapses to the hidden section once every backing layer is off.
- **Hiding the selected layer must clear the selection** so the right panel returns to the guide (matches the pre-feature behavior, where dropping the hidden layer cleared the panel). Details and data-table need a **dedicated effect keyed on `layerHiddenSet`** because their "check selection" effects trigger only on batched/structural changes, not on a visibility toggle; geochart and time-slider fold the check into their existing selection effect via `entry.isHidden`. Guard with `layerHiddenSet[path] && inVisibleRangeSet[path] !== false` (don't clear for out-of-range).
- **`LayerList` renders two separate `<List>` elements** (visible + hidden), each labelled by a heading via `aria-labelledby` (`useId()` for unique ids), plus a `role="status" aria-live="polite"` region announcing moves (diff previous vs current hidden set, skip first render). Keep native `ul`/`li` list semantics (not `role="group"`) so screen readers announce item counts.
- **Items remount when moving between the visible/hidden groups.** The two groups are separate child arrays, so React unmounts/remounts an item that changes group — a **ref goes stale**. Restore focus by the item's stable id via `getGVElementByFullId(mapId, id)` (map-scoped, avoids the raw `document.getElementById` lint rule), not a ref.
- **Non-interactive hidden rows use `tabIndex={-1}` + no `onClick`/`onKeyDown` + `disableRipple` + a cursor/hover-reset style — NOT MUI `disabled`.** `disabled` washes out opacity, suppresses the tooltip, and flips `noWrap`; `tabIndex={-1}` keeps the clean grey/italic look while removing the row from the tab order and click path. The eye `IconButton` stays the only control. Style hidden text to match the Layers panel "not visible" symbology (`grey[700]` + italic on `.layerTitle`).
- **Details app-bar collapsed icon view (`toggleMode`/`compact`).** In that narrow view: suppress hidden layers (drop them in the list builder AND in the second "out-of-range-with-features" merge block), suppress the section headings and the empty-state instructions box (`compact` prop threaded `details-panel → Layout → LayerList`). The auto-guide effect force-reopens the panel and fights the user's close — guard it with a `userDismissedGuideRef` set in `onRightPanelClosed` and reset when the panel closes or features reappear. The `renderCloseButton` in `toggleMode` must show whenever the right panel is open (even without feature content) so users can return to the list.
...
- `#region STATE GETTERS & HOOKS` — For **matched pairs** (getter + hook for the same property). Always list getter first, then hook.
- `#region STATE GETTERS & HOOKS - OTHERS (no match between getter-hook)` — For **orphan hooks** that have no matching getter (hook-only, typically UI-only values).

When adding a getter to a property that was previously hook-only in the OTHERS region, **move both the getter and hook** to the main region. The OTHERS region is exclusively for properties that have no getter.
